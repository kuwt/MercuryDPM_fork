...
The following functions changed their name (the line starting with a minus is the old version, the line starting with + is the new version)
 ...
 CHECK AGAIN OVER THE BOUNDARIES
...
SelfTests/Boundaries/PSDManualInsertionSelfTest.cpp
 - a new test for the ManualInsertion routine of the PSD class
...
Demos/Membrane/MembraneDemo.cpp
 - added a new demo to demonstrate the membrane featuer
...
Kernel/Math/Helpers:
 ...
 - added new function writeCommandLineToFile, so you can store command line arguments (should they be stored in the restart file?)
 - added new function linspace*double, double, int) which works the same waz as in MATLAB
...
Kernel/CG/BaseCG
 - added functions set[X,Y,Z]Grid(min,max,h), that lets you quickly define a grid of given mesh size
...
Kernel/Boundaries/InsertionBoundary
 - added functions insertParticles, that lets you insert particles once, without adding the boundary to the handler
