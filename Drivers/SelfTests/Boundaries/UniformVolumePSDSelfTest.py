...
# open data file
fileName = "UniformVolumePSDSelfTest.data"
...
probN, bins = np.histogram(radius, density=True, bins=100)
r = bins[:-1]
R = bins[1:]
probV = probN * (R**2+r**2) * (R+r)
probV /= sum(probV)
plt.plot(bins[:-1], probV,'-',label='data')
plt.plot(bins[:-1], bins[:-1]**0/np.sum(bins[:-1]**0)/np.sum(probV),'-',label='analytic')
# set plot options
plt.ylabel('vpsd')
plt.xlabel('radius')
plt.legend()
plt.show()
plt.savefig('UniformVolumePSDSelfTest_NonInterpolated.png',dpi=300)
