...
{
    //define domain size
    Mdouble domainLength = 	0.50e-3; //m	4e-4
    Mdouble domainWidth = 	0.50e-3;	//m	2e-4
    Mdouble domainDepth = 	0.50e-3;	//m	2e-4

    //define Temperatures
    Mdouble initialTemperature = 300; //K
    Mdouble finalTemperature = 400; //K

    //define properties of particle-particle contacts
    Mdouble density = 1030;
    Mdouble collisionTime = 3e-4; //standard value = 3e-4 s
    ...
    //c specific heat cap
    species.setHeatCapacity(1090); //J/kg/K
    //K therm cond
    species.setThermalConductivity(23); //W/m/K
    ...
    Mdouble meanRadius = 25e-6; //m
    Mdouble stdRadius = 0.0 * meanRadius;
    pb.setGaussianDistribution(meanRadius,stdRadius);
    //pb.setUniformDistribution(minRadius,maxRadius);
    //pb.createThreeParticleStack (0.9*meanRadius,1.1*meanRadius,1*meanRadius);
