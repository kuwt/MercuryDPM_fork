...
public:
/*! In 's4-5_SimpleShearConstVolume', it will read the restart file for a relaxed
 * configuration from s3_IsoRelax and start doing the shear with constant volume,
...
{
	std::string restartName ("ini_nu_w1.4");
    ...
    problem.mu_slid = 1.0;				//set sliding friction coefficient
    ...
    // alpha and cap_factor are not used for volume constant shear
	problem.stressYYGoal = 500;//this parameter is not used in volume const case
	
    problem.dot_strain_xy = 6e-4;
    
    problem.tmax = 100000;
    problem.setName("phiconst");
    
    problem.setSaveCount(80000);
