...
public:
/*! In 's4_FreeShearStressStrain', it will read the restart file for a relaxed
 * configuration from s3_IsoRelax and start doing the shear with/without stress control,
 * shear rate will be defined by strain-rate input which is du_x/dy = dot_strain_xy =gamma_dot
 * If you don't apply shear, you could also do all the deformation modes as you want 
 * and this can be combined with/without the servo/stress controll in x,y,z and xy directions.
 * */    
    ...
    {		
        ...
        std::cout << "V_shear = " << velocity_xy << std::endl;
...
{
	std::string restartName ("mu0.01-p500-relax");
	...
	problem.stressXYGoal = -10; //  the input for shear stress control should be always negative sign
