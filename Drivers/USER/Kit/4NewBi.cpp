...
int main(int argc UNUSED, char *argv[] UNUSED)
{
	...
	problem.set_ParticleRadius(1.50002/1000 , 1.5/1000);
	problem.set_ParticleDensity(1134,5175); //2500 & 7850 --> glass & steel.
    	double tc=1e-4;
	problem.set_CollisionTime(tc);
	problem.set_WallCOR(0.7);
	problem.set_WallFriction(0.0); //
	problem.set_ParticleCOR(0.4,0.85,0.625); //(11, 22, 12) OR just leave as a single value for equal COR.
	problem.set_BaseCOR(0.6);
	
	problem.set_switch_plate_amplitude(3.0,0.777/1000); //(time, new amplitude)
...
//This is fake lowering the frequence and raising amplitude by a factor of 5.
    ...
    problem.set_Amplitude(0.777/1000); //normally 0.862
