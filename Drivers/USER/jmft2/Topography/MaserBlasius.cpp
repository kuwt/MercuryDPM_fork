/* MaserBlasius - As with MaserBlasius but using a Maser boundary for
 * upstream conditions. */
#include "Mercury2D.h"
...
class MaserBlasius : public Mercury2D {
    public:

        /* Constructor */
        MaserBlasius(std::string parsfile)
        {
            /* Reading config file */
        ...
        void setupInitialConditions() 
        {
            ...
            back->set(Vec3D(-1, 0, 0), 
                      Vec3D(pars.at("xmin") - pars.at("reservoirLength") - 2*pars.at("particleRadius"), 0, 0)
                     );
...
//                heater->set2D(
//                        Vec3D(pars.at("xmin") - pars.at("reservoirLength"), pars.at("reservoirHeight") * 0.8, 0.0), 
//                        Vec3D(pars.at("xmin"), pars.at("reservoirHeight") * 1.2, 0.0), 
//                        pars.at("reservoirTemperature") );
                heater->set2D(
                          Vec3D(pars.at("xmin") - 2./3. * pars.at("reservoirLength"), 
                              pars.at("reservoirHeight") * 0.8, 0.0), 
                          Vec3D(pars.at("xmin") - 1./3. * pars.at("reservoirLength"), 
...
int main(const int argc, char* argv[]) {
    ...
    if (argc > 1)
    {
        MaserBlasius* problem = new MaserBlasius(argv[1]);
        argv[1] = argv[0];
