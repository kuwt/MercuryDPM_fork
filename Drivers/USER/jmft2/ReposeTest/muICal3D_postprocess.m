...
seriesname = 'sidewalls';
speciesname = 'sidewalls';
angles = 10;
...
phis = masses ./ (domainlength * domainwidth * depths);
...
inertials = (5*diam)/(2*domainlength * domainwidth) * (xmoms) ./ (depths.^5 .* cos(angles * pi/180) .* phis.^3).^(1/2);
...
muICal3D_results = struct(...
    'seriesname', seriesname, 'speciesname', speciesname, ...
    'domainlength', domainlength, 'domainwidth', domainwidth, 'diam', diam, ...
