...
public:
    ...
    void setupInitialConditions() override {
        ...
        // Strain
        isoStrainDot = -100e-8 / getTimeStep(); // Fast for the first part
    ...
    void createWalls()
    {
        ...
        speciesWall -> setDensity(densityParticle);
        speciesWall -> setConstantRestitution(true);
        speciesWall -> setCollisionTimeAndRestitutionCoefficient(collisionTimeSmallestMass, restitutionCoefficient, 1);
        speciesWall -> setUnloadingStiffnessMax(species->getLoadingStiffness() * unLoadingStiffnessMax / loadingStiffness);
        speciesWall -> setCohesionStiffness(species-> getLoadingStiffness() * cohesionStiffness / loadingStiffness);
        speciesWall -> setPenetrationDepthMax(penetrationDepthMax);

        speciesWall -> setSlidingFrictionCoefficient(slidingFrictionCoefficient);
        speciesWall -> setSlidingStiffness(species -> getLoadingStiffness()*2.0/7.0);
        speciesWall -> setSlidingDissipation(species -> getDissipation()*2.0/7.0);
        speciesWall -> setRollingFrictionCoefficient(slidingRollingCoefficient);
        speciesWall -> setRollingStiffness(species -> getLoadingStiffness()*2.0/7.0);
        speciesWall -> setRollingDissipation(species -> getDissipation()*2.0/7.0);
        speciesWall -> setTorsionFrictionCoefficient(slidingTorsionCoefficient);
        speciesWall -> setTorsionStiffness(species -> getLoadingStiffness()*2.0/7.0);
        speciesWall -> setTorsionDissipation(species -> getDissipation()*2.0/7.0);
        ...
        wall.set(Vec3D(0, 0, 1), Vec3D(0, 0, wallPositionZ));
        WN = wallHandler.copyAndAddObject(wall);

        wall.set(Vec3D(0, 0, -1), Vec3D(0, 0, -wallPositionZ));
        WS = wallHandler.copyAndAddObject(wall);

        wall.set(Vec3D(1, 0, 0), Vec3D(wallPositionX, 0, 0));
        WE = wallHandler.copyAndAddObject(wall);

        wall.set(Vec3D(-1, 0, 0), Vec3D(-wallPositionX, 0, 0));
        WW = wallHandler.copyAndAddObject(wall);

        wall.set(Vec3D(0, 1, 0), Vec3D(0, wallPositionY, 0));
        WF = wallHandler.copyAndAddObject(wall);

        wall.set(Vec3D(0, -1, 0), Vec3D(0, -wallPositionY, 0));
        WB = wallHandler.copyAndAddObject(wall);
    ...
    void moveWalls()
    {
        ...
        WN->set(Vec3D(0, 0, 1), Vec3D(0, 0, wallPositionZ));

        WS->set(Vec3D(0, 0, -1), Vec3D(0, 0, -wallPositionZ));

        WE->set(Vec3D(1, 0, 0), Vec3D(wallPositionX, 0, 0));

        WW->set(Vec3D(-1, 0, 0), Vec3D(-wallPositionX, 0, 0));

        WF->set(Vec3D(0, 1, 0), Vec3D(0, wallPositionY, 0));

        WB->set(Vec3D(0, -1, 0), Vec3D(0, -wallPositionY, 0));
    }
    ...
    void computeStress()
    {
        ...
        //totalStressWall = ( WN -> getForce().getLength() + WE -> getForce().getLength() + WF -> getForce().getLength()
        //                  + WS -> getForce().getLength() + WW -> getForce().getLength() + WB -> getForce().getLength() ) /
        //                        ( 8 * (wallPositionX*wallPositionY + wallPositionX*wallPositionZ + wallPositionY*wallPositionZ) );

        totalStressWall = (WN -> getForce().getLength() + WS -> getForce().getLength())/(24*wallPositionX*wallPositionY)
                          + (WE -> getForce().getLength() + WW -> getForce().getLength())/(24*wallPositionZ*wallPositionY)
                          + (WF -> getForce().getLength() + WB -> getForce().getLength())/(24*wallPositionZ*wallPositionY);


        uniStressWall = ( WN -> getForce().getLength() + WS -> getForce().getLength() ) / ( 8 * wallPositionX*wallPositionY);
    }
    ...
    void makeIsoCFile()
    {
        ...
        isoCFile <<
                 ...
                 "Stress" << std::setw(16) <<
                 "StressWall" << std::setw(16) <<
                 "uniStress" << std::setw(15) <<
                 "uniStressWall" << std::setw(15) <<
                 "E_ratio" << std::setw(12) <<
                 "cN" << std::setw(12) <<
                 "vR" << std::setw(12) <<
    ...
    void writeToIsoCFile()
    {
        isoCFile <<
                 ...
                 std::fixed << std::setprecision(2) << std::setw(16) << totalStressWall <<
                 std::fixed << std::setprecision(2) << std::setw(16) << uniStress <<
                 std::fixed << std::setprecision(2) << std::setw(16) << uniStressWall <<
...
private:
    ...
    Mdouble totalStress;
    Mdouble uniStress;
    Mdouble totalStressWall;
    Mdouble uniStressWall;
...
int main(){
    ...
    script.penetrationDepthMax = 0.3;
