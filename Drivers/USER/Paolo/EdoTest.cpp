...
public:
     ...
     */
        ...
        // Strain
        isoStrainDot = -100; // should be something like 1e-8/timeStep but for the first part it is set faster
        isoStrainDot2 = -1e-8/getTimeStep(); // should be something like 1e-8/timeStep
    ...
    void actionsBeforeTimeStep() override
    {
        ...
        if (stage == 1) {
            ...
            if (voidRatio < 0.45)
            {
                ...
                std::cout << "isoStrainDot2 è: " << isoStrainDot2 << std::endl;
                w0.set(Matrix3D(0,0,0,0,0,0,0,0,0), Matrix3D(isoStrainDot2, 0, 0, 0, isoStrainDot2, 0, 0, 0, isoStrainDot2), Matrix3D(0,0,0,0,0,0,0,0,0), true);
        ...
        if (stage == 2) {
            ...
            {
                boundaryHandler.clear();
                w0.set(Matrix3D(0,0,0,0,0,0,0,0,0), Matrix3D(0, 0, 0, 0, 0, 0, 0, 0, isoStrainDot2), Matrix3D(0,0,0,0,0,0,0,0,0), true);
                boundaryHandler.copyAndAddObject(w0);
                stage = 3;
            }
        }
    ...
    void moveParticles(){
        ...
        for (auto& p : particleHandler)
        {
            ...
            p->move( Vec3D(isoStrainDot2 * getTimeStep() * relativeToCenter.X,
                           isoStrainDot2 * getTimeStep() * relativeToCenter.Y,
                           0) );
        }
    }
...
int main(){
    ...
    script.radiusCluster = 10 * script.radiusParticle; // Empiricamente: SETTARLO CON SICUREZZA!!!!!!!!!!
    ...
    script.cdatOutputTimeInterval = script.getTimeStep() * 3000;
    script.setSaveCount( floor(script.cdatOutputTimeInterval/script.getTimeStep()) );
    script.setTimeMax(9000*script.cdatOutputTimeInterval);
