...
public:
    ...
    void setupInitialConditions() override {
        ...
        logger(WARN,"N %",particleHandler.getNumberOfRealObjects());
        readRestartFile("PerS1P1F1safe");
        logger(ERROR,"N %",particleHandler.getNumberOfRealObjects());
        // NAME SETTING
        std::ostringstream name;
        name << "InterclusterRelaxMPIS1P1F1";
        setName(name.str());
    ...
    {
        logger(INFO,"NT %",getNumberOfTimeSteps());
        if (stage == 1) {
            makeDataAnalysis();
            moveWalls();
            ...
            if ( uniStress > 3e5 && getTime() - t0 > 1e5 * getTimeStep()) {
                writeToIsoCFile();
                writeToIsoCFile();
                t0 = getTime();
                stage = 2;
        ...
        if (stage == 2) {
            ...
            if (getTime()-t0 > 1e5*getTimeStep()){ // 20 printings
                stage = 3;
            }
        }
    ...
    void printTime() const override
    {
        ...
        switch (stage)
        {
            ...
            case 2: std::cout << "Relaxation: ";
                break;

            case 3: std::cout << "Isotropic compression: ";
                break;

            case 4: std::cout << "Relaxation: ";
                break;

            case 5: std::cout << "Isotropic compression: ";
                break;

            case 6: std::cout << "Uniaxial compression: ";
                break;
    ...
    void setClusterSpecies()
    {
        ...
        for (int k = 0; k < nClusters; ++k) {
            for (int i = 0; i < 300; ++i) {
                particleHandler.getObject(k*300+i)->setSpecies(speciesHandler.getObject(k));
            }
        }
    ...
    void createWalls()
    {
        wallPositionX = 1.61473102745109e-05;
        wallPositionY = 1.61473102745109e-05;
        wallPositionZ = 1.61473102745109e-05;

        WNS = dynamic_cast<PeriodicBoundary*>(boundaryHandler.getObject(0));
        WEW = dynamic_cast<PeriodicBoundary*>(boundaryHandler.getObject(1));
        WFB = dynamic_cast<PeriodicBoundary*>(boundaryHandler.getObject(2));
    ...
    void computeCoordinationNumberAndOverlaps()
    {
        ...
        meanForceOnInteraction/= interactionHandler.getNumberOfObjects();
        meanRelativeOverlap /= interactionHandler.getNumberOfObjects();
        averageCubicRelativeOverlap = cbrt( averageCubicRelativeOverlap);
        averageCubicRelativeOverlap /= interactionHandler.getNumberOfObjects();
    }
    ...
    void makeIsoCFile()
    {
        ...
        cdatName << getName() << PROCESSOR_ID << ".isoc";
...
int main(){
    ...
    script.setNumberOfDomains({1,1,2});
