...
#include "DoublePorosityStressStrainB.h"

DoublePorosityStressStrainB::DoublePorosityStressStrainB(){
...
DoublePorosityStressStrainB::DoublePorosityStressStrainB(std::vector<Mdouble> radC, std::vector<Vec3D> posC, Mdouble pDM, Mdouble ExpuSMintra, Mdouble rExpCSIntra, Mdouble sFC, Mdouble bS, std::string n){
    radiiCluster = radC;
    ...
    loadingStiffnessInter      = 20 * unLoadingStiffnessMaxIntra;
    relativeTangentialStiffness = 1;
...
void DoublePorosityStressStrainB::setupInitialConditions(){
    ...
    primoValore = true;
    results.reserve(100);
    nonRelativeResults.reserve(100);
...
void DoublePorosityStressStrainB::actionsAfterTimeStep(){
    ...
    if ( stage == 4 && getTime()-t0 > 100*fileOutputTime)
    {
        makeDataAnalysis();
        makeOverlFile();
        writeOverlFile();
    ...
    if ( stage == 6 && e <= eValues[counterCompression] )
    {
        makeDataAnalysis();
        if (primoValore){
            stressE0 = uniStress;
        ...
        nonRelativeResults.push_back(uniStress);
        counterCompression++;
    }
    ...
    if ( e <= eValues[99]) {
        ...
        std::ostringstream nonRelativeResultsName;
        nonRelativeResultsName << getName() << ".results2";
        nonRelativeResultsFile.open(nonRelativeResultsName.str(), std::ios::out);
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            nonRelativeResultsFile <<
                        std::fixed << std::setprecision(3) << nonRelativeResults[i] <<
                        std::endl;
        }
...
void DoublePorosityStressStrainB::printTime() const{
...
void DoublePorosityStressStrainB::setSpecies() {
    ...
    species -> setCohesionStiffness(species->getLoadingStiffness()*cohesionStiffness/unLoadingStiffnessMaxIntra);
    ...
    for (int i = 0; i < nClusters; ++i) {
        ...
        speciesVector[i] -> setSlidingFrictionCoefficient(0);
    ...
    for (int i = 0; i < nClusters; ++i) {
        for (int j = 0; j < nClusters; ++j) {
            if (i!=j){
                ...
                speciesHandler.getMixedObject(speciesVector[i], speciesVector[j]) -> setSlidingFrictionCoefficient(0);
...
void DoublePorosityStressStrainB::setSpecies2() {
...
void DoublePorosityStressStrainB::makeWalls(){
...
void DoublePorosityStressStrainB::makeIsoCFile() {
...
void DoublePorosityStressStrainB::makeOverlFile() {
...
void DoublePorosityStressStrainB::dampVelocities()
{
...
void DoublePorosityStressStrainB::makeDataAnalysis()
{
...
//! Computes coordination number and overlaps
void DoublePorosityStressStrainB::computeCoordinationNumberAndOverlaps()
{
...
void DoublePorosityStressStrainB::computeVoidRatioAndPorosity()
{
...
//! computes stress
void DoublePorosityStressStrainB::computeStress()
{
    stressXX = 0.0;
    stressYY = 0.0;
    stressZZ = 0.0;
...
//! Computes void dimension
void DoublePorosityStressStrainB::computeVoidDimensions()
{
    bool inserted = false;
...
bool DoublePorosityStressStrainB::particleInsertionSuccessful(Mdouble r) {
    ...
    while (insertionFailCounter < 1e4)
    {
...
void DoublePorosityStressStrainB::writeOverlFile() {
...
void DoublePorosityStressStrainB::writeIsoCFile() {
...
void DoublePorosityStressStrainB::writeToIsoCFileTarget()
{
    isoCFile <<
...
void DoublePorosityStressStrainB::makeContactModelFile(){
