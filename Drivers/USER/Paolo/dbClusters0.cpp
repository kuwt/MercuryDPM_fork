...
public: //  ----- FUNCTIONS AND VARIABLES --------------------------------------
      ...
      for(int i = 0; i < gridLength; i++)
      {
         ...
         for(int j = 0; j < gridLength; j++)
         {
            ...
            for(int k = 0; k < gridLength; k++)
            {
               ...
               // sets the position of the fictitious particle to the grid point
      ...
      for (int i = 0; i < numberOfClusters; i++)
      {
         ...
         while(nParticlesInserted < numberOfParticlesPerCluster)
         {
            ...
            else
            {
               ...
               std::cout << "MAYBE TRY TO CHANGE THE clusterSizeSafetyFactor VARIABLE"<< std::endl;
               std::cout << "QUITTING"<< std::endl;
   ...
   void makeDataAnalysis()
   {
      ...
      for (int i=particleHandler.getNumberOfObjects()-1; i>=0; i--)
       {
         distanceFromForceCenter = particleHandler.getObject(i) -> getPosition() - centerOfMass;
      ...
      for (std::vector<BaseInteraction*>::const_iterator i = interactionHandler.begin(); i != interactionHandler.end(); ++i)
      {
         ...
         // piston interactions
         if (stage >= 5 && (*i) -> getI() -> getIndex() == pistonPointer -> getIndex())
         {
   ...
   void setBoxSize()
   {
      ...
      // the box size containing he cluster is rescaled according to the user defined clusterSizeSafetyFactor
   ...
   void setMixedSpeciesCohesionMatrix()
   {
      ...
      // prints the ratio between collision time and time step for a mixed particle-wall interaction (should be > 50 to be super-precise)
       std::cout << mixedSpeciesMatrix[0][0] -> getCollisionTime(massParticle) << std::endl << std::endl;
...
{
   // TIME PARAMETERS
   double timeStep = 0.5e-5;        // DEM solver integration time step
   double timeMax = 10;        // tMax at which the simulaton gets shut down
   ...
   double phiParticle = 0.1;        // plasticity depth
   ...
   // GRANULES PARAMETERS
   int numberOfParticlesPerCluster = 300;         // number of particles per cluster
   ...
   int nZLatticeSites = 1;       // number of initial cluster lattice sites in z direction
   ...
   // {
         ...
         name.clear();
         std::cout.unsetf(std::ios::floatfield);
         /*! Modifico il nome per semplicità nel lanciare gli output
