...
public:
    ...
    void setupInitialConditions() override {
        ...
        readRestartFile("BasePerS1P1F1");
        cdatOutputTimeInterval = getTimeStep() * 500;
        setSaveCount( floor(cdatOutputTimeInterval/getTimeStep()) );
        setTimeMax(1000);

        // NAME SETTING
        std::ostringstream name;
        name << "RestartedPerS1P1F1";
        setName(name.str());

        std::cout << "Le particelle sono: " << particleHandler.getSize() << std::endl;
        ...
        for (int i = 0; i < nClusters; ++i) {
            for (int j = 0; j < nParticles; ++j) {
                particleHandler.getObject(nParticles*i+j)->setSpecies(speciesVector[i]);
            }
        }
    ...
    void createWalls()
    {
        ...
        wall.set(Vec3D(0, 0, 1), getZMin(), getZMax());
        WNS = boundaryHandler.copyAndAddObject(wall);

        wall.set(Vec3D(1, 0, 0), getXMin(), getXMax());
        WEW = boundaryHandler.copyAndAddObject(wall);

        wall.set(Vec3D(0, 1, 0), getYMin(), getYMax());
    ...
    void moveWalls()
    {
        ...
        setXMin(-wallPositionX);
        setYMin(-wallPositionY);
        setZMin(-wallPositionZ);
        setXMax(wallPositionX);
        setYMax(wallPositionY);
        setZMax(wallPositionZ);
    ...
    void makeIsoCFile()
    {
        ...
        std::ostringstream isocName;
        isocName << getName() << ".isoc";

        isoCFile.open(isocName.str(), std::ios::out);
...
int main(){
    ...
    Mdouble boxSize = 4.4375e-5; //6e-5 per farne 11
    ...
    script.cdatOutputTimeInterval = script.getTimeStep() * 500;
