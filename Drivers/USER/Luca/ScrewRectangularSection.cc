...
// Default constructor
ScrewRectangularSection::ScrewRectangularSection()
{
    start_.setZero();
    l_ = 1.0;
    rMax_ = 1.0;
    ...
    logger(DEBUG, "ScrewRectangularSection() constructor finished.");
}

// Copy constructor
ScrewRectangularSection::ScrewRectangularSection(const ScrewRectangularSection& other)
    : BaseWall(other)
{
    start_ = other.start_;
    l_ = other.l_;
    ...
    logger(DEBUG, "ScrewRectangularSection(const ScrewRectangularSection&) copy constructor finished.");
}
...
ScrewRectangularSection::ScrewRectangularSection(Vec3D start, Mdouble l, Mdouble R, Mdouble n, Mdouble omega, Mdouble thickness)
{
    start_ = start;
    l_ = l;
    rMax_ = R;
    ...
    logger(DEBUG, "ScrewRectangularSection(Vec3D, Mdouble, Mdouble, Mdouble, Mdouble, Mdouble) constructor finished.");
}

// Destructor
ScrewRectangularSection::~ScrewRectangularSection()
{
    logger(DEBUG, "~ScrewRectangularSection() finished, destroyed the screw.");
}

// A pointer to the copy of the screw
ScrewRectangularSection* ScrewRectangularSection::copy() const
{
    return new ScrewRectangularSection(*this);
}

// Function to set the screw parameters
void ScrewRectangularSection::set(Vec3D start, Mdouble length, Mdouble bladeRadius, Mdouble numberOfTurns, Mdouble omega, Mdouble thickness)
{
    start_ = start;
    l_ = length;
    rMax_ = bladeRadius;
...
void ScrewRectangularSection::setRadius(Mdouble bladeRadius)
{
    rMax_ = bladeRadius;
}

// Function to change the thickness (and the half-thickness as well)
void ScrewRectangularSection::setThickness(Mdouble thickness)
{
    thickness_ = thickness;
    delta_ = 0.5 * thickness_;
}
...
void ScrewRectangularSection::setOmega(Mdouble omega)
{
    omega_ = omega;
}

// Function to make the screw rotate by adding \omega*t to the angular offset
void ScrewRectangularSection::rotate(Mdouble dt)
{
    offset_ += omega_ * dt;
}

// Function that returns the current angular offset
Mdouble ScrewRectangularSection::getOffset()
{
    return offset_;
}

// Function to change the screw angular offset
void ScrewRectangularSection::setOffset(Mdouble off)
{
    offset_ = off;
}

// Function to make the screw rotate by incrementing the angular offset
void ScrewRectangularSection::incrementOffset(Mdouble off)
{
    offset_ += off;
}

// Input stream
void ScrewRectangularSection::read(std::istream& is)
{
    BaseWall::read(is);
    std::string dummy;
    is >> dummy >> start_
...
void ScrewRectangularSection::oldRead(std::istream& is)
{
    std::string dummy;
    is >> dummy >> start_
    >> dummy >> l_
...
// Writer
void ScrewRectangularSection::write(std::ostream& os) const
{
    BaseWall::write(os);
    os << " start " << start_
    << " Length " << l_
...
// Returns the string "ScrewRectangularSection"
std::string ScrewRectangularSection::getName() const
{
    return "ScrewRectangularSection";
}

// The contact detection algorithm
// All the quantities in cylindrical coordinates are referred to the screw axis
bool ScrewRectangularSection::getDistanceAndNormal(const BaseParticle& p, Mdouble& distance, Vec3D& normal_return) const
{
...
BaseInteraction*
ScrewRectangularSection::getInteractionWith(BaseParticle* p, unsigned timeStamp, InteractionHandler* interactionHandler)
{
    Mdouble distance;
    Vec3D normal;
