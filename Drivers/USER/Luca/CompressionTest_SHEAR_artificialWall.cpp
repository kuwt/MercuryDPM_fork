...
01.11:
- added the artificial bladed piston

26.11:
- added the if (mu_S > 0.99) condition
- re-introduced the pre-shearing stage
- added the condition if (chilling) rotatePiston;
- running the simulations in "slow mode" (velocities scaling = 1.0, chilling/waiting time = 0.1)

29.11
- the *0.99 velocity rescale condition during the pre-shear is unfeasible, it won't converge: pre-shear taken out
*/
...
private:
   ...
   void actionsAfterTimeStep() override
   {
      ...
      if (fmod(getTime(), cdatOutputTimeInterval) < getTimeStep())
      {
         ...
         ", <d_tot> = " << meanTotalRelativeOverlap <<
         ", max(d_tot) = " << maxTotalRelativeOverlap <<
         std::endl;
      }
   }
...
public:
   ...
   void setTorqueCalibrationPoints(int n, double *pressureArrayPointer, double *torqueArrayPointer, double pauseDuration, double pauseDuration2)
   {
      ...
      muSCalibratedArray = new double[n];
   ...
   void setDentedPistonParameters(double height, double thickness, double ratio)
   {
      dentsHeight = height;
      dentsThickness = thickness;
      dentLengthOpeningRatio = ratio;
   }
   ...
   void makePiston()
   {
      ...
      w1.set(pistonHeight, dentsHeight, 0.5*dentsThickness, 0.0);
      w1pointer = wallHandler.copyAndAddObject(w1);

      // wall y=0 [+]
      w2.setSpecies(speciesBig);
      w2.set(pistonHeight, dentsHeight, 0.5*dentsThickness, 0.5*constants::pi);
   ...
   void makeDataAnalysis()
   {
      ...
      for (std::vector<BaseInteraction*>::const_iterator i = interactionHandler.begin(); i != interactionHandler.end(); ++i)
      {
         ...
         if ((*i) -> getI() -> getIndex() == w1pointer -> getIndex())
         {
            ...
            pistonTorque -= ((*i) -> getContactPoint()).X * ((*i) -> getForce()).Y - ((*i) -> getContactPoint()).Y * ((*i) -> getForce()).X;
            // std::cout << "w1 " << ((*i) -> getForce()).Z << "   " << ((*i) -> getContactPoint()).X * ((*i) -> getForce()).Y - ((*i) -> getContactPoint()).Y * ((*i) -> getForce()).X << std::endl;
   ...
   void updateTargetDataPoint()
   {
      ...
      if (calibrationStep == 0) previousTargetPressure = 0.0;
      else previousTargetPressure = pressureSteps[calibrationStep - 1];
   ...
   void movePiston()
   {
      ...
      w1pointer -> move(pistonVelocity*getTimeStep());
      w2pointer -> move(pistonVelocity*getTimeStep());
   ...
   void rotatePiston()
   {
      ...
      w1pointer -> rotate(pistonAngularVelocity*getTimeStep());
      w2pointer -> rotate(pistonAngularVelocity*getTimeStep());
   ...
   void wigglePiston(double t0) // check
   {
      ...
      w1pointer -> rotate(wigglingAngularVelocity*getTimeStep());
      w2pointer -> rotate(wigglingAngularVelocity*getTimeStep());
   ...
   void runTorqueCalibration()
   {
         ...
         if (fabs((pistonTorque - targetTorque)/targetTorque) > maxTorqueRelDeviation)
         {
            ...
            if (fmod(getTime() - t0, nTimeStepsBetweenStiffnessAdjustments*getTimeStep()) < getTimeStep())
            {
               ...
               if (previousTargetPressure < targetPressure)
               {
                  ...
                  // sanity check to keep mu_S < 1
                  if (bigBigSlidingFrictionCoeff > 0.99 && targetTorque > pistonTorque)
                  {
               ...
               else
               {
                  ...
                  if (kCBig < 0.01 && targetTorque < pistonTorque)
                  {
      ...
      // continues rotating the piston during the chilling stage
      if (isPressureTargetMet && isTorqueTargetMet) rotatePiston();
      ...
      {
         // keeps track of teh number of compression and decompression steps ran
         if (previousTargetPressure < targetPressure) nCompressionSteps++;
         else nDecompressionSteps++;

         // updates the mean value of friction and cohesion at the calibrated data points
         kCCalibratedArray[calibrationStep] = kCBig;
         muSCalibratedArray[calibrationStep] = bigBigSlidingFrictionCoeff;
   ...
   {
      // compute the friction variations if compression steps were made
      meanMuSBig = 0.0;
      meanRelativeDeviationMuSBig = 0.0;
      meanRelativeDisplacementMuSBig = 0.0;

      if (nCompressionSteps)
      {
         for (int i = 0; i < nCompressionSteps; i++) meanMuSBig += muSCalibratedArray[i];
         meanMuSBig /= nCompressionSteps;

         for (int i = 0; i < nCompressionSteps; i++) meanRelativeDeviationMuSBig += fabs(muSCalibratedArray[i] - meanMuSBig);
         meanRelativeDeviationMuSBig /= nCompressionSteps*meanMuSBig;
      ...
      if (nDecompressionSteps)
      {
         for (int i = 0; i < nDecompressionSteps; i++) meanKCBig += kCCalibratedArray[i];
         meanKCBig /= nDecompressionSteps;

         for (int i = 0; i < nDecompressionSteps; i++) meanRelativeDeviationKCBig += fabs(kCCalibratedArray[i] - meanKCBig);
         meanRelativeDeviationKCBig /= nDecompressionSteps*meanKCBig;
   ...
   void makeCdatFile()
   {
      ...
      cdatFile << "time \t Eel \t Ekin/Eel \t h \t v \t omega \t P \t Pbase \t Pcyl \t torque \t torque_target \t |(T - Ttarg)/Ttarg| \t dTotMean \t dTotMax \t dPistonMean \t dPistonMax \t dBaseMean \t dBaseMax \t mu_S \t k1 \t k2 \t kC \t phi" << std::endl;
   }
   ...
   void printTime() const override
   {
      ...
      else
      {
         ...
         "mu_S = " << bigBigSlidingFrictionCoeff << ", k1 = " << std::setprecision(6) << std::left << std::setw(10) << speciesBig -> getLoadingStiffness() <<
   ...
   // complex piston
   CompressionPistonSurface w1;
   CompressionPistonSurface* w1pointer;
   CompressionPistonSurface w2;
   CompressionPistonSurface* w2pointer;
   double dentsHeight;
   ...
   int calibrationStep;
   int nCompressionSteps;
   int nDecompressionSteps;
...
int main(int argc, char *argv[])
{
   ...
   std::string dataPointsFileNameCOMPRESSION = "Calibration_dataPoints/Calibration_dataPoints_COMPRESSION_CAPSULAC60___RESCALED_05.dat";
   std::string dataPointsFileNameSHEAR = "Calibration_dataPoints/Calibration_dataPoints_SHEAR_CAPSULAC60.dat";
   ...
   double pauseBeforeTorqueCalibration = 0.1;
   ...
   double pistonAngularVelocity = 1.0*pistonRealVelocity;
   ...
   double pistonDentsHeight = 8.0e-4;
   double pistonDentsThickness = casingRadius/bigToCasingSizeRatio/2.0; // 100
   double pistonDentsLengthOpeningRatio = 0.25;
   ...
   double k1Big = 1555.0;
   double kCBig = 1.0;
   double phiBig = 0.0253517; // pow(1.0 - k1Small/k2MaxSmall, 2.0)/(k1Small/k2MaxSmall)*deltaHatMax[0]/(cConstant[0] - 1.0)
   ...
   do
   {
      ...
      name.clear();
      std::cout.unsetf(std::ios::floatfield);
      name << "ShearTest_" << material << "_" << phase << "_Nrun_" << runNumber <<
      ...
      "_muS_" << std::fixed << std::setprecision(2) << initialMuSArray[runNumber] << "_muR_" << muRBigBig << "_DEBUG_1_omega_2_thickness_2_bladed_artificialWall";
      ...
      crShear.setDentedPistonParameters(pistonDentsHeight, pistonDentsThickness, pistonDentsLengthOpeningRatio);
