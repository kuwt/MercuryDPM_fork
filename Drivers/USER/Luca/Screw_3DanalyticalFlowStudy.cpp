/*
 *** PERIODIC SCREW FEEDER - 3D ANALYTICAL FLOW STUDY ***
 Particles are placed inside a paralelepipedal casing and let settle under gravity over the screw.
 When they are still they are cut around the casing size and the casing is inserted.
 When still again the screw is set in motion.
...
class Screw3DFlowStudy : public Mercury3D
{
private:
    ...
    {
        // stage 1: the particles are loaded and let to settle
        if (stage == 1 && getKineticEnergy()/getElasticEnergy() < 1.e-4)
        {
//            std::cout << "Destroying the screw. It will be re-created after loading the .restart file...\n";
//            destroyScrew();
//            std::cout << "DONE!\n";
        ...
        // stage 2: the particles are cut outside of hte casing and the casing is loaded
        if (stage == 2 && getKineticEnergy()/getElasticEnergy() < 1.e-4 && restartedFile)
        {
...
// make functions
    ...
    void makeDataAnalysis()
    {
        ...
        for (std::vector<BaseInteraction*>::const_iterator i = interactionHandler.begin(); i != interactionHandler.end(); ++i)
        {
            ...
            if (((*i) -> getOverlap())/(particleHandler.getObject((*i) -> getP() -> getIndex()) -> getRadius()) > maxRelativeOverlap) maxRelativeOverlap = ((*i) -> getOverlap())/(particleHandler.getObject((*i) -> getP() -> getIndex()) -> getRadius());

            // torque computation
            if ((*i) -> getI() -> getIndex() == helicoidPointer -> getIndex())
            {
                totalTorque += ((*i) -> getContactPoint()).X*((*i) -> getForce()).Y - ((*i) -> getContactPoint()).Y*((*i) -> getForce()).X;
            }
...
//  ----- GLOBAL FUNCTIONS -----
    ...
    {
//        if (stage == 2 && !restartedFile) return false;
        if (stage == 4 && getTime() > getTimeMax()) return false;
...
int main(int argc, char *argv[])
{
   ...
   double screwLength = 0.1;
   double screwCasingRadius = 0.015;
   double screwBladeRadius = 0.015;
   double screwShaftRadius = 0.005;
   double screwBladeThickness = 0.0005;
   int nOfScrewTurns = 2;


    // the blade-to-particle-radius ratio
    double radiusRatio = 15;
    double packingFraction;

    // the velocity steps array
    const int nVelocitySteps = 4;
    double velocityArray[nVelocitySteps] = {constants::pi, 2.0*constants::pi, 3.0*constants::pi, 4.0*constants::pi};
    double durationOfVelocityIncrement = 15.0;

    // friction coefficients
    double particleParticleSlidingFriction = 0.3;
    double particleParticleRollingFriction = 0.05;
    double particleWallSlidingFriction = 0.3;
    double particleWallRollingFriction = 0.05;

//    problem.restartedFile = true;
	...
	// updating the cycled variables
	packingFraction = 0.60;

	// Problem setup
	Screw3DFlowStudy problem;

//        // sets the bool restartedFile true or false accorging to the argc value
//        if (argc > 1) {problem.restartedFile = true;}
//        else {problem.restartedFile = false;}
	...
	problem.setParticleRadiusAndDispersity((screwBladeRadius - screwShaftRadius)/radiusRatio,0.1);
	problem.setParticleDensity(1500.);

	// sets the stiffnesses
	problem.setStiffnesses(2000.0, 5000.0);

	// sets interaction parameters
	problem.setRestitutionCoefficients(0.5, 0.8);    // p-p and p-w restitution coefficients
	problem.setSlidingFrictionCoefficients(particleParticleSlidingFriction, particleWallSlidingFriction);   // p-p and p-w sliding friction coefficients
	problem.setRollingFrictionCoefficients(particleParticleRollingFriction, particleWallRollingFriction);   // p-p and p-w rolling friction coefficients
	...
	problem.setScrewGeometry(screwLength, screwCasingRadius, screwBladeRadius, screwShaftRadius, nOfScrewTurns, screwBladeThickness);
	problem.setScrewOrigin(Vec3D(0.,0.,0.));

	// screw operational parameters
	problem.setVelocitySteps(nVelocitySteps, velocityArray, durationOfVelocityIncrement);
	...
	problem.setBoxToScrewWidthRatio(true, 1.1); // if true manually sets the box-to-screw size ratio
	problem.setInterParticleLoadingLatticeRelativeDistance(0.1);   // relative to particle radius
	problem.developmentHackParticles(false, 20);    // if true only inserts the specified amount of particle layers (for testing and debugging)
	...
	// reads teh arguments from command line (if any) and starts the simulation
	problem.readArguments(argc, argv);

	std::ostringstream fileName;
	std::cout.unsetf(std::ios::floatfield);
	fileName << "Screw_3DFlowStudy_rr_" << radiusRatio << "_packingFraction_" << std::fixed << std::setprecision(2) << packingFraction <<
	"_muSpp_" << particleParticleSlidingFriction << "_muSpw_" << particleWallSlidingFriction << "_muRpp_" << particleParticleRollingFriction <<
  "_muRpw_" << particleWallRollingFriction << "_omega_" << velocityArray[0];
	problem.setName(fileName.str());

//        // name setting
//        if (problem.restartedFile)
//        {
//            std::ostringstream nameOnRestart;
//            std::cout.unsetf(std::ios::floatfield);
//            nameOnRestart << "Screw_3DFlowStudy_rr_" << radiusRatio << "_packingFraction_" << std::fixed << std::setprecision(2) << packingFraction;
//            problem.setName(nameOnRestart.str());
//        }
//        else
//        {
//            std::ostringstream name;
//            std::cout.unsetf(std::ios::floatfield);
//            name << "Screw_3DFlowStudy_Settling_rr_" << radiusRatio;
...
//        problem.dataFile.setFileType(FileType::MULTIPLE_FILES_PADDED);
	...
	// computes the total duration of the simulation and prints the output
