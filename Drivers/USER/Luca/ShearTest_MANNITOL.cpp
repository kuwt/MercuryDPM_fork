...
//    - scrapped kConly mode
//    - correct .kdata file print
// 12.04.19 - trying out the q0 +- 20% deviation condition
//    - added the flag at the beginning
//    - WARNING: got rid of the muR output -> possible incompatibility!
...
public:
   ...
   void setConstrainedMode(bool cm)
   {
      constrainedMode = cm;
   }
   ...
   void makePiston()
   {
      ...
      pistonHeight = (particleHandler.getHighestPositionComponentParticle(2) -> getPosition()).Z + 1.1*(particleHandler.getLargestParticle() -> getRadius()) + bladesHeight;
      ...
      {
         w.set(pistonHeight, bladesHeight, 0.5*bladesThickness, constants::pi*n/numberOfBlades);
         ...
         if (fabs((pistonTorque - targetTorque)/targetTorque) > maxTorqueRelDeviation)
         {
            ...
            if (fmod(getTime() - t0, nTimeStepsBetweenParametersAdjustments*getTimeStep()) < getTimeStep())
            {
               ...
               if (previousTargetPressure < targetPressure)
               {
                 ...
                 if (((constrainedMode && bigBigSlidingFrictionCoeff < 0.8*pointerToInitialMuSArray[nCurrentRoutineRun]) || bigBigSlidingFrictionCoeff < 0.001) && targetTorque < pistonTorque)
                 {
                 ...
                 if (((constrainedMode && bigBigSlidingFrictionCoeff > 1.2*pointerToInitialMuSArray[nCurrentRoutineRun]) || bigBigSlidingFrictionCoeff > 0.50) && targetTorque < pistonTorque)
                 {
               ...
               else
               {
                  ...
                  if (((constrainedMode && kCBig < 0.8*pointerToInitialKCArray[nCurrentRoutineRun]) || kCBig < 0.001) && targetTorque < pistonTorque)
                  {
                  ...
                  if (((constrainedMode && kCBig > 1.2*pointerToInitialKCArray[nCurrentRoutineRun]) || kCBig > 5.0*k2MaxBig) && targetTorque < pistonTorque)
                  {
...
int main(int argc, char *argv[])
{
   ...
   double pistonAngularVelocity = 100.0*pistonRealVelocity;

   double preShearingDuration = 0.5;
   double preShearingOscillationAmplitude = constants::pi/200.0;
   double preShearingOscillationFrequency = 1.0/0.001;
   ...
   double pistonBladesHeight = 7.0e-4;
   ...
   double maximumTorqueRelativeDeviation = 1.0e-3;
   ...
   do
   {
      ...
      "_muS_" << std::fixed << std::setprecision(2) << initialMuSArray[runNumber] << "_nD_" << std::fixed << std::setprecision(0) << numberOfPistonBlades;
      if (fastMode) name << "_FAST";
      if (constrainedMode) name << "_CONSTRAINED";
      ...
      crShear.setConstrainedMode(constrainedMode);
