...
bool Helicoid03::getDistanceAndNormal(const BaseParticle& p, Mdouble& distance, Vec3D& normal_return) const
{
    ...
    else if (rho >= maxR_ && fabs(deltaZ)*cosEta >= delta_) // REGION 3
    {
        ...
        if (normalDistance > p.getWallInteractionRadius()*cosGamma) return false;
...
std::vector<BaseInteraction *> Helicoid03::getInteractionWith(BaseParticle* p, unsigned timeStamp, InteractionHandler* interactionHandler)
{
    Mdouble distance;
    Vec3D normal;
