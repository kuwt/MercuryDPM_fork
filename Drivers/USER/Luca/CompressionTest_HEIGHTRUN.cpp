...
void readExperimentalData(int nTot, std::string inputFileName, double *hArray, double *pArray)
{
...
int main(int argc, char *argv[])
{
   ...
   int nDataPointsToAnalyze = 6;
   int nTotalDataPoints = 17;
   std::string dataPointsFileName = "Calibration_COMPRESSION_experimentalData";
   ...
   readExperimentalData(nTotalDataPoints, dataPointsFileName, heightArray, pressureArray);

   // finding the initial P=0 point by interpolation
   double deltaHatMax = (particleBedHeight - heightArray[5])/particleBedHeight;
   // double cConstant = (particleBedHeight - heightArray[5])/(heightArray[nTotalDataPoints - 1] - heightArray[5]);
   // double cConstant = 6.54;
   double cConstant = 3.68299;
   ...
   else if (nDataPointsToAnalyze == 16) phase = "FULL";
   else phase = "";
   ...
   // the multiple calibration runs loop
   do
   {
      // --- TESTING FUNCTIONS ---
      ...
      // problem.readRestartFile(argv[2]);
      ...
      problem.setCasingProperties(casingRadius, particleBedHeight, densityWalls);
      ...
      problem.setStiffnessCalibrationDataPoints(nDataPointsToAnalyze, heightArray, pressureArray, pauseDuration);
      ...
      "___cConstant_" << cConstant << "_DhatMax_" << deltaHatMax << "_phiDynamicallyTuned";
