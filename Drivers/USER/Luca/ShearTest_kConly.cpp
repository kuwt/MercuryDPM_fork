...
17.02.19
- transformed into kConly mode
- encoded rescaled blade
- added max phi = 1.0 check
*/
...
public:
   ...
   void setCalibrationRoutineParameters(int nRun, int nMax, double *kCArray, double *devKCArray, double *dispKCArray)
   {
   ...
   void runTorqueCalibration()
   {
      ...
      if (isPressureTargetMet && !isTorqueTargetMet && getTime() > t0 + pauseBeforeTorqueCalibration)
      {
         if (fabs((pistonTorque - targetTorque)/targetTorque) > maxTorqueRelDeviation)
         {
            ...
            if (fmod(getTime() - t0, nTimeStepsBetweenParametersAdjustments*getTimeStep()) < getTimeStep())
            {
               ...
               if (kCBig < 0.1 && targetTorque < pistonTorque)
               {
      ...
      {
         // updates the mean value of cohesion at the calibrated data points
         kCCalibratedArray.push_back(kCBig);
...
void initializeCalibrationArrays(int nRunsMax, double *initKC, double *devKCArray, double *dispKCArray, double kCBigInit)
{
...
int main(int argc, char *argv[])
{
   ...
   double maximumParametersRelativeDeviation = 0.10;
   ...
   initializeCalibrationArrays(nMaximumRoutineCycles, initialKCArray, deviationKCArray, displacementKCArray, kCBig);
   ...
   do
   {
      ...
      name.clear();
      std::cout.unsetf(std::ios::floatfield);
      name << "ShearTest_kConly_" << material << "_Nrun_" << runNumber <<
      ...
      "_muS_" << std::fixed << std::setprecision(2) << muSBigBig << "_muR_" << muRBigBig << "_nD_" << numberOfPistonBlades << "_rf_" << pistonBladeRescaleFactor;
      ...
      crShear.setCalibrationRoutineParameters(runNumber, nMaximumRoutineCycles, initialKCArray, deviationKCArray, displacementKCArray);
   ...
   while (runNumber < nMaximumRoutineCycles && !(displacementKCArray[runNumber - 1] < maximumParametersRelativeDeviation && deviationKCArray[runNumber - 1] < maximumParametersRelativeDeviation));
