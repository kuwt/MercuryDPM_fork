...
// Default constructor
DualFeederCasing_BOT::DualFeederCasing_BOT()
{
   start_.setZero();
   l_ = 1.0;
   r_ = 1.0;
   ...
   logger(DEBUG, "DualFeederCasing_BOT() constructor finished.");
}

// Copy constructor
DualFeederCasing_BOT::DualFeederCasing_BOT(const DualFeederCasing_BOT& other)
: BaseWall(other)
{
   start_ = other.start_;
   l_ = other.l_;
   ...
   logger(DEBUG, "DualFeederCasing_BOT(const DualFeederCasing_BOT&) copy constructor finished.");
}

// Initialized constructor
// Parameters in: initial starting point of the axis, length, radius, screw axes distance.
DualFeederCasing_BOT::DualFeederCasing_BOT(Vec3D start, Mdouble length, Mdouble radius, Mdouble distance)
{
   start_ = start;
   l_ = length;
   r_ = radius;
   ...
   logger(DEBUG, "DualFeederCasing_BOT(Vec3D, Mdouble, Mdouble, Mdouble) constructor finished.");
}

// Destructor
DualFeederCasing_BOT::~DualFeederCasing_BOT()
{
   logger(DEBUG, "~DualFeederCasing_BOT() finished, destroyed the screw.");
}

// A pointer to the copy of the casing
DualFeederCasing_BOT* DualFeederCasing_BOT::copy() const
{
   return new DualFeederCasing_BOT(*this);
}

// Function to set the casing parameters
void DualFeederCasing_BOT::set(Vec3D start, Mdouble length, Mdouble radius, Mdouble distance)
{
   start_ = start;
   l_ = length;
   r_ = radius;
   ...
   std::cout << "\n\nBOT casing crew parameters set\n";
...
void DualFeederCasing_BOT::setRadius(Mdouble radius)
{
   r_ = radius;
}

// Function to set the distance
void DualFeederCasing_BOT::setDistance(Mdouble distance)
{
   delta_ = distance;
}

// Input stream
void DualFeederCasing_BOT::read(std::istream& is)
{
   BaseWall::read(is);
   std::string dummy;
   is >> dummy >> start_
...
void DualFeederCasing_BOT::oldRead(std::istream& is)
{
   std::string dummy;
   is >> dummy >> start_
   >> dummy >> l_
...
// Writer
void DualFeederCasing_BOT::write(std::ostream& os) const
{
   BaseWall::write(os);
   os << " Start " << start_
   << " Length " << l_
...
// Returns the string "DualFeederCasing_BOT"
std::string DualFeederCasing_BOT::getName() const
{
   return "DualFeederCasing_BOT";
}

// The contact detection algorithm
bool DualFeederCasing_BOT::getDistanceAndNormal(const BaseParticle& p, Mdouble& distance, Vec3D& normal_return) const
{
   ...
   // if the particle lays above the screw axes there is no collision
   if (p.getPosition().Y > start_.Y) return false;

   // if the particle is outside the length containing the screw there is no collision
   const Mdouble interactionRadius = p.getWallInteractionRadius(this);
   if (p.getPosition().Z > l_ + start_.Z + interactionRadius) return false;
   if (p.getPosition().Z < start_.Z - interactionRadius) return false;
...
DualFeederCasing_BOT::getInteractionWith(BaseParticle* p, Mdouble timeStamp, InteractionHandler* interactionHandler)
{
   Mdouble distance;
   Vec3D normal;
