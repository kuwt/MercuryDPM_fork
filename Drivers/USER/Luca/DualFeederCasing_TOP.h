...
#ifndef DualFeederCasing_TOP_H
#define DualFeederCasing_TOP_H
...
class DualFeederCasing_TOP : public BaseWall
{
public:

    // Default constructor
    DualFeederCasing_TOP();

    // Copy constructor
    DualFeederCasing_TOP(const DualFeederCasing_TOP& other);

    // Initialized constructor
    DualFeederCasing_TOP(Vec3D start, Mdouble length, Mdouble radius, Mdouble distance);

    // Destructor
    ~DualFeederCasing_TOP();

    // Pointer to a copy of the casing
    DualFeederCasing_TOP* copy() const final;
