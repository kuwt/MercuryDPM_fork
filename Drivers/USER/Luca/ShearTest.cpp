...
NOTES:
28.01.19
- corrected an error in the evaluation of the deviations for friction and cohesion
- rewriting as a function of <vectors> instead of arrays

29.01.19
- introduced the averaged torque computation
- increased the number of time steps between parameters tuning to 10 (from 5)
- removed the computation of teh mean coordination number (unused). *** IF IT IS TO BE REINTRODUCED, IT SHOULD BE REWORKED AND PUT INSIDE THE INTERACTION LOOP ***

30.01.19
- introduced the "fast mode"
- introduced the wiggling mode on initial compression
- cleaned and removed unused data analysis variables linked to base and cylinder interactions

08.02.19
- rewritten and corrected the .kdata output file

04_03_19
- re-corrected the output in the .kdata output file
- added a cout of the number of calibrated array points as check
- added pistonBladesRescaleFactor parameter
- added kConly as a flag
- added careful implementation of fast mode (now skipping hte pause after torque met)
- commented-out the powder bed vertical cut in fast mode (unstable)
*/
...
public:
   ...
   void setBladedPistonParameters(int n, double height, double rescaleFactor, double thickness)
   {
      numberOfBlades = n;
      bladesHeight = height;
      pistonBladeRescaleFactor = rescaleFactor;
      bladesThickness = thickness;
   }
   ...
   void makePiston()
   {
      std::cout << "CREATING BLADED PISTON" << std::endl;

      pistonHeight = (particleHandler.getHighestPositionComponentParticle(2) -> getPosition()).Z + 1.1*(particleHandler.getLargestParticle() -> getRadius()) + bladesHeight*pistonBladeRescaleFactor;
      ...
      {
         w.set(pistonHeight, bladesHeight*pistonBladeRescaleFactor, 0.5*bladesThickness, constants::pi*n/numberOfBlades);
         bladesArray.push_back(wallHandler.copyAndAddObject(w));
      }
   }
   ...
   void makeDataAnalysis()
   {
      ...
      for (std::vector<BaseInteraction*>::const_iterator i = interactionHandler.begin(); i != interactionHandler.end(); ++i)
      {
         ...
         for (int n = 0; n < numberOfBlades; n++)
         {
            if ((*i) -> getI() -> getIndex() == bladesArray[n] -> getIndex())
            {
               pistonPressure -= ((*i) -> getForce()).Z;
               pistonTorque += ((*i) -> getContactPoint()).X * ((*i) -> getForce()).Y - ((*i) -> getContactPoint()).Y * ((*i) -> getForce()).X;
            }
      ...
      averagedTorque.push_back(fabs(pistonTorque)/pistonBladeRescaleFactor);
   ...
   void movePiston()
   {
      ...
      for (int n = 0; n < numberOfBlades; n++) bladesArray[n] -> move(pistonVelocity*getTimeStep());
   ...
   void rotatePiston()
   {
      ...
      for (int n = 0; n < numberOfBlades; n++) bladesArray[n] -> rotate(pistonAngularVelocity*getTimeStep());
   ...
   void wigglePiston(double t0)
   {
      ...
      for (int n = 0; n < numberOfBlades; n++) bladesArray[n] -> rotate(wigglingAngularVelocity*getTimeStep());
   ...
   void runTorqueCalibration()
   {
      ...
      if (isPressureTargetMet && !isTorqueTargetMet && getTime() > t0 + pauseBeforeTorqueCalibration)
      {
         if (fabs((pistonTorque - targetTorque)/targetTorque) > maxTorqueRelDeviation)
         {
            ...
            if (fmod(getTime() - t0, nTimeStepsBetweenParametersAdjustments*getTimeStep()) < getTimeStep())
            {
               ...
               // if in kConly mode immediately switch to the "else"
               if (previousTargetPressure < targetPressure && !kConly)
               {
               ...
               else
               {
                  ...
                  // sanity check to keep kC < 10kE
                  if (kCBig > 10.0*k2MaxBig && targetTorque > pistonTorque)
                  {
         ...
         {
            if (!fastMode) std::cout << "Torque target reached, now chillin' for " << pauseAfterDataPointMet << " seconds\n";
            else std::cout << "Torque target reached, no pause since in fast mode\n";
            t0 = getTime();
            isTorqueTargetMet = true;
         }
      }
      ...
      if (isPressureTargetMet && isTorqueTargetMet && (getTime() > t0 + pauseAfterDataPointMet || fastMode))
      {
         // updates the mean value of friction and cohesion at the calibrated data points
         if (previousTargetPressure < targetPressure && !kConly) muSCalibratedArray.push_back(bigBigSlidingFrictionCoeff);
   ...
   void computeVariations()
   {
      ...
      // if in kConly mode the mean muS is set as the initial muS, to setup correctly the next iteration of the cycle
      if (kConly) meanMuSBig = bigBigSlidingFrictionCoeff;
   ...
   void makeKdataFile()
   {
      ...
      for (int i = 0; i < nCalibrationDataPoints; i++)
      {
         kdataFile << pressureSteps[i] << "\t" << torqueSteps[i] << "\t";

         if (!kConly)
         {
            if (i == 0)
            {
               kdataFile << "mu_S = " << muSCalibratedArray[0] << std::endl;
               muSarrayIndex++;
            }
            else
            {
               if (pressureSteps[i] > pressureSteps[i-1])
               {
                  kdataFile << "mu_S = " << muSCalibratedArray[muSarrayIndex] << std::endl;
                  muSarrayIndex++;
               }
               else
               {
                  kdataFile << "kC = " << kCCalibratedArray[kCarrayIndex] << std::endl;
                  kCarrayIndex++;
               }
            }
         }
         ...
         {
            kdataFile << "kC = " << kCCalibratedArray[i] << std::endl;
         }
      }

      kdataFile.close();

      std::cout << "FINAL CHECK ON CALIBRATED ARRAYS SIZE" << std::endl;
      std::cout << "Total calibration data points: " << nCalibrationDataPoints << std::endl;
      std::cout << "muS array index: " << muSarrayIndex << std::endl;
      std::cout << "kC array index: " << kCarrayIndex << std::endl << std::endl;
   }
   ...
   // // applies changes if ran in fast mode
   // void applyFastMode()
   // {
   //    std::cout << "*** WARNING - RUNNING IN FAST MODE ***" << std::endl;
   //    std::cout << "Resizing particle bed to 3/5 its initial height" << std::endl;
   //    std::cout << "Initial number of particles: " << particleHandler.getNumberOfObjects() << std::endl;
   //
   //    double hPmax = (particleHandler.getHighestPositionComponentParticle(2) -> getPosition()).Z;
   //
   //    for (int i = 0; i < particleHandler.getNumberOfObjects(); i++)
   //    {
   //       if (particleHandler.getObject(i) -> getPosition().Z + particleHandler.getObject(i) -> getRadius() > 0.6*hPmax) particleHandler.removeObject(i);
   //    }
   //
   //    std::cout << "Final number of particles: " << particleHandler.getNumberOfObjects() << std::endl;
   // }
   ...
   int numberOfBlades;
   double bladesHeight;
   double pistonBladeRescaleFactor;
   double bladesThickness;
   std::vector<CompressionPistonSurface*> bladesArray;
...
int main(int argc, char *argv[])
{
   ...
   std::string dataPointsFileNameCOMPRESSION = "Calibration_dataPoints/Calibration_dataPoints_COMPRESSION_MANNITOL___RESCALED_05.dat";
   std::string dataPointsFileNameSHEAR = "Calibration_dataPoints/Calibration_dataPoints_SHEAR_MANNITOL.dat";
   ...
   // PISTON BLADES PARAMETERS
   double numberOfPistonBlades = 4;
   double pistonBladesHeight = 1.0e-4;
   double pistonBladeRescaleFactor = 2.0;
   ...
   do
   {
      ...
      "_muS_" << std::fixed << std::setprecision(2) << initialMuSArray[runNumber] << "_muR_" << muRBigBig <<
      "_nD_" << std::fixed << std::setprecision(1) << numberOfPistonBlades << "_rf_" << pistonBladeRescaleFactor;
      if (kConly) name << "_kcONLY";
      if (fastMode) name << "_FAST";
      ...
      crShear.setFastMode(fastMode);
      crShear.setKcOnlyMode(kConly);
      ...
      crShear.setBladedPistonParameters(numberOfPistonBlades, pistonBladesHeight, pistonBladeRescaleFactor, pistonBladesThickness);
