...
#ifndef MultiStepCompressionTestDataDrivenNewMethod_H
#define MultiStepCompressionTestDataDrivenNewMethod_H
...
private:
    ...
    void actionsAfterTimeStep() override
    {
        ...
        {
            if (setsInserted == 0) std::cout << "Inserting the particles...\n";
...
//            previousTargetHeight = pistonHeight;
...
//            std::cout << "HERE\n";
//            
//            if (fmod(getTime(),cdatOutputTimeInterval) < getTimeStep() && getTime() > 0.1)
//            {
//                std::cout << "HERE\n";
...
// ----- FUNCTIONS CALLED IN THE MAIN -----
    ...
    void setCalibrationDataPoints(int n, double *heightArrayPointer, double *pressureArrayPointer, double duration)
    {
        compressionInstances = n;
        compressionInstancePauseDuration = duration;
        ...
        if (verbose)
        {
            std::cout << "\tCalibration height-pressure steps:\n";
	...
	// sets teh pointer to update the calibration status
	void setCalibrationStatus(bool *calibrationPointer)
	{
		calibrationStatusPointer = calibrationPointer;
	}
    
    // set the maximum number of maximum number of routine runs
    void setMaxRunNumber(int n)
    {
        maximumNumberOfRoutineRuns = n;
    }
    
    // sets the actual routine run number
    void setRunNumber(int n)
    {
        runNumber = n;
    }
    
    // sets the value of hte minimum convergence threshold
    void setPressureConvergengeThresholdValue(double value)
    {
        convergenceThresholdPressure = value;
    }
    
    // allocates the calibration data to a pointer for further usage
    void setInitialStiffnessPointer(double *pointerK1, double *pointerKC)
    {
        pointerToInitialK1 = pointerK1;
        pointerToInitialKC = pointerKC;
        initialK1 = pointerToInitialK1[runNumber];
        initialKC = pointerToInitialKC[runNumber];
    }
...
//        totalHeight = 0.5*casingHeight + 2.1*meanRadiusBig*(1.0 + dispersityBig)*numberOfLevels;
   ...
   // runs the data point calibration routine
   void calibrateDataPoints()
   {
       // halves and reverses the piston velocity if needed
        ...
        // if the height is met and the pressure is not met tunes the stiffness
        if (heightTargetMet && !pressureTargetMet)
        {
            // if teh pressure is far from the targeted one tune the stiffness 
            if (fabs((pistonPressure - targetPressure)/targetPressure) > convergenceThresholdPressure)
            {
                // in compression tune k1, in decompression tune kC
                if (previousTargetHeight >= targetHeight) adjustK1();
                else adjustKC();
                
                stiffnessWasAdjusted = true;
            }
            else
            {
                std::cout << "Pressure target reached.\n";
                
                // if the stiffness was adjusted updates the initial stiffness for the next run and restarts the caliration
                if (stiffnessWasAdjusted)
                {
                    std::cout << "The stiffness was adjusted.\n";
                    std::cout << "Former values: k1 = " << initialK1 << "  ,  kC = " << initialKC << "\n";
                    std::cout << "Calibrated values: k1 = " << particleStiffnessBig << "  ,  kC = " << particleCohesiveStiffness << "\n";
                    
                    if (runNumber < maximumNumberOfRoutineRuns - 1)
                    {
                        std::cout << "Updating the new starting stiffness.\n";
                        if (previousTargetHeight >= targetHeight)
                        {
                            pointerToInitialK1[runNumber + 1] = 0.5*(initialK1 + particleStiffnessBig);
                            pointerToInitialKC[runNumber + 1] = initialKC;
                        }
                        else
                        {
                            pointerToInitialK1[runNumber + 1] = initialK1;
                            pointerToInitialKC[runNumber + 1] = 0.5*(initialKC + particleCohesiveStiffness);
                        }
                        
                        std::cout << "The new starting values will be: k1 = " << pointerToInitialK1[runNumber + 1] << "  ,  kC = " << pointerToInitialKC[runNumber + 1] << "\n";
                ...
                else // else switches to the next height target
                {
                    std::cout << "The initial stiffness were fine.\n";
                    std::cout << "Now chillin' for " << compressionInstancePauseDuration << " seconds...\n";
                    timePlaceholder = getTime();
        ...
        // when both height and pressure are met updates the target values
        if (heightTargetMet && pressureTargetMet && getTime() > timePlaceholder + compressionInstancePauseDuration)
        {
            compressionStep++;
...
// ----- VARIABLES -----
    ...
    double *pointerToInitialK1;
    double *pointerToInitialKC;
    ...
    int maximumNumberOfRoutineRuns;
    int runNumber;
    ...
    bool loadingBranchDone;
    bool reloadingBranchDone;
    bool unloadingBranchDone;
    bool *pointerToInitialBools;
    bool stiffnessWasAdjusted = false;
    ...
    double compressionInstancePauseDuration;
    double convergenceThresholdPressure;
    bool *calibrationStatusPointer;
};
