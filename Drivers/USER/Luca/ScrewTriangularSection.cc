...
#include "ScrewTriangularSection.h"
...
// Default constructor
ScrewTriangularSection::ScrewTriangularSection()
{
    start_.setZero();
    l_ = 1.0;
    rMax_ = 1.0;
    ...
    logger(DEBUG, "ScrewTriangularSection() constructor finished.");
}

// Copy constructor
ScrewTriangularSection::ScrewTriangularSection(const ScrewTriangularSection& other)
    : BaseWall(other)
{
    start_ = other.start_;
    l_ = other.l_;
    ...
    logger(DEBUG, "ScrewTriangularSection(const ScrewTriangularSection&) copy constructor finished.");
}
...
ScrewTriangularSection::ScrewTriangularSection(Vec3D start, Mdouble l, Mdouble R, Mdouble r, Mdouble n, Mdouble omega, Mdouble thickness)
{
    start_ = start;
    l_ = l;
    rMax_ = R;
    ...
    logger(DEBUG, "ScrewTriangularSection(Vec3D, Mdouble, Mdouble, Mdouble, Mdouble, Mdouble, Mdouble) constructor finished.");
}

// Destructor
ScrewTriangularSection::~ScrewTriangularSection()
{
    logger(DEBUG, "~ScrewTriangularSection() finished, destroyed the screw.");
}

// A pointer to the copy of the screw
ScrewTriangularSection* ScrewTriangularSection::copy() const
{
    return new ScrewTriangularSection(*this);
}

// Function to set the screw parameters
void ScrewTriangularSection::set(Vec3D start, Mdouble length, Mdouble bladeRadius, Mdouble shaftRadius, Mdouble numberOfTurns, Mdouble omega, Mdouble thickness)
{
    start_ = start;
    l_ = length;
    rMax_ = bladeRadius;
...
void ScrewTriangularSection::setRadius(Mdouble bladeRadius)
{
    rMax_ = bladeRadius;
}

// Function to change the thickness (and the half-thickness as well)
void ScrewTriangularSection::setThickness(Mdouble thickness)
{
    thickness_ = thickness;
    delta_ = 0.5 * thickness_;
}
...
void ScrewTriangularSection::setOmega(Mdouble omega)
{
    omega_ = omega;
}

// Function to make the screw rotate by adding \omega*t to the angular offset
void ScrewTriangularSection::rotate(Mdouble dt)
{
    offset_ += omega_ * dt;
}

// Input stream
void ScrewTriangularSection::read(std::istream& is)
{
    BaseWall::read(is);
    std::string dummy;
    is >> dummy >> start_
...
void ScrewTriangularSection::oldRead(std::istream& is)
{
    std::string dummy;
    is >> dummy >> start_
    >> dummy >> l_
...
// Writer
void ScrewTriangularSection::write(std::ostream& os) const
{
    BaseWall::write(os);
    os << " start " << start_
    << " Length " << l_
...
// Returns the string "ScrewTriangularSection"
std::string ScrewTriangularSection::getName() const
{
    return "ScrewTriangularSection";
}

// The contact detection algorithm
// All the quantities in cylindrical coordinates are referred to the screw axis
bool ScrewTriangularSection::getDistanceAndNormal(const BaseParticle& p, Mdouble& distance, Vec3D& normal_return) const
{
...
//    if ((deltaN + lambda*rho)*cosGamma > p.getWallInteractionRadius(this)) return false;
    if ((deltaN + lambda*deltaR)*cosGamma > p.getWallInteractionRadius(this)) return false;
...
BaseInteraction*
ScrewTriangularSection::getInteractionWith(BaseParticle* p, unsigned timeStamp, InteractionHandler* interactionHandler)
{
    Mdouble distance;
    Vec3D normal;
