...
private:
   ...
   void actionsAfterTimeStep() override
   {
      ...
      if (fmod(getTime(), 5.0*cdatOutputTimeInterval) < getTimeStep())
      {
         ...
         ", k1 = " << speciesBig -> getLoadingStiffness() <<
...
public:
   ...
   void movePiston()
   {
      ...
      w1pointer -> moveAlongZ(pistonVelocity*getTimeStep());
      // w1Bpointer -> moveAlongZ(pistonVelocity*getTimeStep());
      w2pointer -> moveAlongZ(pistonVelocity*getTimeStep());
      // w2Bpointer -> moveAlongZ(pistonVelocity*getTimeStep());
      // w3pointer -> moveAlongZ(pistonVelocity*getTimeStep());
      // w3Bpointer -> moveAlongZ(pistonVelocity*getTimeStep());
      // w4pointer -> moveAlongZ(pistonVelocity*getTimeStep());
      // w4Bpointer -> moveAlongZ(pistonVelocity*getTimeStep());
      // pistonPointer -> move(Vec3D(0.0,0.0,pistonHeight));
...
int main(int argc, char *argv[])
{
   ...
   double k1Big = 892.0;
   double kCBig = 1.0;
   double phiBig = 0.0232912; // pow(1.0 - k1Small/k2MaxSmall, 2.0)/(k1Small/k2MaxSmall)*deltaHatMax[0]/(cConstant[0] - 1.0)
   ...
   do
   {
      ...
      "_muS_" << std::fixed << std::setprecision(2) << initialMuSArray[runNumber] << "_muR_" << muRBigBig << "_DEBUG_1_omega_2_thickness";
