...
#include "DualFeederCasing_BOT.h"
...
public:
   ...
   void setBoundaries()
   {
      setXMin(loadingBoxMin.X);
      setYMin(loadingBoxMin.Y);
      setZMin(loadingBoxMin.Z);
   ...
   void makeBoundaries()
   {
      ...
      wall.set(Vec3D(-1.,0.,0.),Vec3D(getXMin(),0.,0.)); // getXMin()
      wallHandler.copyAndAddObject(wall);
      wall.set(Vec3D(1.,0.,0.),Vec3D(getXMax(),0.,0.)); // getXMax
      wallHandler.copyAndAddObject(wall);

      // wall.set(Vec3D(0,-1.,0.),Vec3D(0.,-.03,0.)); // getYMin()
      // wallHandler.copyAndAddObject(wall);
      ...
      DualFeederCasing_BOT temporaryBottomCasing;
      temporaryBottomCasing.setSpecies(specieWall);
      temporaryBottomCasing.set(Vec3D(0.0,0.0,0.0), getZMax() - getZMin(), .02, .01);
      bottomCasing = wallHandler.copyAndAddObject(temporaryBottomCasing);
   }
   ...
   void makeScrews()
   {
      Helicoid05 temporaryScrewR;
      temporaryScrewR.setSpecies(specieWall);
      temporaryScrewR.set(Vec3D(-.01,0.0,0.0), getZMax() - getZMin(), .015, .003, 1.0, constants::pi, .0001, true);
      // temporaryScrewR.setOffset(0.5*constants::pi);
      screwR = wallHandler.copyAndAddObject(temporaryScrewR);

      Helicoid05 temporaryScrewL;
      temporaryScrewL.setSpecies(specieWall);
      temporaryScrewL.set(Vec3D(.01,0.0,0.0), getZMax() - getZMin(), .015, .003, 1.0, constants::pi, .0001, true);
      screwL = wallHandler.copyAndAddObject(temporaryScrewL);
   }
   ...
   void makeCasing()
   {
      ...
      if(!fixedPackingFraction)
      {
         ...
         {
            if (particleHandler.getObject(i) -> getPosition().X > 0.0)
            {
               if (pow(particleHandler.getObject(i) -> getPosition().X - 0.01, 2.) +
               pow(particleHandler.getObject(i) -> getPosition().Y - 0.0, 2.) >
               pow(.018 - .95*(particleHandler.getObject(i) -> getRadius()),2.)) particleHandler.removeObject(i);
            }
            else
            {
               if (pow(particleHandler.getObject(i) -> getPosition().X + 0.01, 2.) +
      ...
      DualFeederCasing_TOP temporaryTopCasing;
      temporaryTopCasing.setSpecies(specieWall);
      temporaryTopCasing.set(Vec3D(0.0,0.0,0.0), getZMax() - getZMin(), .02, .01);
      topCasing = wallHandler.copyAndAddObject(temporaryTopCasing);
   ...
   void makeRotation()
   {
      screwL -> rotate(getTimeStep());
      screwR -> rotate(getTimeStep());
   }
   ...
   Helicoid05 *screwL, *screwR;
   DualFeederCasing_BOT *bottomCasing;
   DualFeederCasing_TOP *topCasing;
...
int main(int argc, char *argv[])
{
   ...
   problem.setPackingFraction(false, packingFraction);   // if true forces the second value to be the packing fraction after the casing insertion
   ...
   problem.setScrewGeometry(26.8/1000.0, 22.0/1000.0, 19.7/1000.0, 13.1/1000.0, 1.0, 0.9/1000.0);
   problem.setScrewOrigin(Vec3D(0.,0.,0.));
   problem.setScrewVelocity(constants::pi);
   problem.setScrewFillingRatio(fillingRatio);
   problem.setScrewRunningTime(10.0);  // this will overwrite the value set by setTimeMax once the screw starts turning
