...
// Default constructor
ScrewRoundEdges::ScrewRoundEdges()
{
    start_.setZero();
    l_ = 1.0;
    rMax_ = 1.0;
    ...
    logger(DEBUG, "ScrewRoundEdges() constructor finished.");
}

// Copy constructor
ScrewRoundEdges::ScrewRoundEdges(const ScrewRoundEdges& other)
    : BaseWall(other)
{
    start_ = other.start_;
    l_ = other.l_;
    ...
    logger(DEBUG, "ScrewRoundEdges(const ScrewRoundEdges&) copy constructor finished.");
}
...
ScrewRoundEdges::ScrewRoundEdges(Vec3D start, Mdouble l, Mdouble R, Mdouble n, Mdouble omega, Mdouble thickness)
{
    start_ = start;
    l_ = l;
    rMax_ = R;
    ...
    logger(DEBUG, "ScrewRoundEdges(Vec3D, Mdouble, Mdouble, Mdouble, Mdouble, Mdouble) constructor finished.");
}

// Destructor
ScrewRoundEdges::~ScrewRoundEdges()
{
    logger(DEBUG, "~ScrewRoundEdges() finished, destroyed the Screw.");
}

// A pointer to the copy of the screw
ScrewRoundEdges* ScrewRoundEdges::copy() const
{
    return new ScrewRoundEdges(*this);
}

// Function to set the screw parameters
void ScrewRoundEdges::set(Vec3D start, Mdouble length, Mdouble bladeRadius, Mdouble numberOfTurns, Mdouble omega, Mdouble thickness)
{
    start_ = start;
    l_ = length;
    rMax_ = bladeRadius;
...
void ScrewRoundEdges::setRadius(Mdouble bladeRadius)
{
    rMax_ = bladeRadius;
}

// Function to change the thickness (and the half-thickness as well)
void ScrewRoundEdges::setThickness(Mdouble thickness)
{
    thickness_ = thickness;
    delta_ = 0.5 * thickness_;
}
...
void ScrewRoundEdges::setOmega(Mdouble omega)
{
    omega_ = omega;
}

// Function to make the screw rotate by adding \omega*t to the angular offset
void ScrewRoundEdges::rotate(Mdouble dt)
{
    offset_ += omega_ * dt;
}

// Input stream
void ScrewRoundEdges::read(std::istream& is)
{
    BaseWall::read(is);
    std::string dummy;
    is >> dummy >> start_
...
void ScrewRoundEdges::oldRead(std::istream& is)
{
    std::string dummy;
    is >> dummy >> start_
    >> dummy >> l_
...
// Writer
void ScrewRoundEdges::write(std::ostream& os) const
{
    BaseWall::write(os);
    os << " start " << start_
    << " Length " << l_
...
// Returns the string "ScrewRoundEdges"
std::string ScrewRoundEdges::getName() const
{
    return "ScrewRoundEdges";
}

// The contact detection algorithm
// All the quantities in cylindrical coordinates are referred to the screw axis
bool ScrewRoundEdges::getDistanceAndNormal(const BaseParticle& p, Mdouble& distance, Vec3D& normal_return) const
{
    ...
    if (deltaN > p.getWallInteractionRadius() + delta_) return false;
...
ScrewRoundEdges::getInteractionWith(BaseParticle* p, unsigned timeStamp, InteractionHandler* interactionHandler)
{
    Mdouble distance;
    Vec3D normal;
