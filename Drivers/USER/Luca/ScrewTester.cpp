...
public:
   ...
   void makeBoundaries()
   {
      ...
      wall.set(Vec3D(0,-1.,0.),Vec3D(0.,screwBladeRadius - 2.0*particleRadius,0.)); // getYMin()
      wallHandler.copyAndAddObject(wall);
   ...
   void makeScrew()
   {
      ...
      shaft -> addObject(Vec3D(-1.,0.,0.), Vec3D(0.5*screwShaftRadius,0.,0.));
      ...
      helicoid = wallHandler.copyAndAddObject(ScrewCC());
      helicoid -> setSpecies(specieWall);
      helicoid -> set(screwOrigin, screwLength, screwBladeRadius, screwShaftRadius, screwNumberOfTurns, screwAngularVelocity, screwThickness);
   ...
   void makeRotation()
   {
      ...
      helicoid -> incrementOffset(screwAngularVelocity*getTimeStep());
