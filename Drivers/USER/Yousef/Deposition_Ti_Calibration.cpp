...
public:
    //
    Mdouble scale = 1e-1; //1e-1; //copy to speed and simulation boundaries
    //
    //
    //Mdouble TimeRemoveBackWall = 0.2;
    Mdouble TimeRemoveWall = 0.2;
    Mdouble TimeMoveTool = 0.25;
    ...
    void setupInitialConditions() {
        ...
        // my parameters:
        Mdouble particleRadius = 35.144e-4 / 2.0;//D(50) = 35.144e-4 um;
        Mdouble fillVolume = (90*scale*scale)*(((ToolWidth/scale)-10e-1)*scale)*(Gap); //0.0018 cm3
        //-------------------------------------------------------------------------- Anthony's code--------------------------------------------------------------------------
        //Step 1 : setup
        std::vector<int> particlesPerBin(5);
        std::vector<double> sizeRanges(6);

        int numberOfParticles;
        int numberOfParticlesToInsert;
        numberOfParticles=static_cast<int>(fillVolume/(particleRadius*particleRadius*particleRadius*4.0/3.0*constants::pi));
        /* Orginal data from company
         * 0.6 percent less than 2mm
         * 22.8 percent less than 2-4mm
         * 65.5 percent less than 4-6mm
         * 9.6 percent less than 6-8mm
         *
         * Will add the extra fake data to complete teh information
         * 0 percent less than 1mm
         * (1.5%) 100 percent less than 10mm
         */
        particlesPerBin[0]=0.006*numberOfParticles;
        particlesPerBin[1]=0.227*numberOfParticles;
        particlesPerBin[2]=0.655*numberOfParticles;
        particlesPerBin[3]=0.096*numberOfParticles;
        particlesPerBin[4]=0.015*numberOfParticles;

        logger(INFO,"You will create % particles",numberOfParticles);
         ...
         * Bin 4 has rnage 4-5
         * Bin 5 has range 5-6
         * Hence range mean of 3,4 is the medium particles size and the one the users sets.
         */

        // Now rest are from the experimental data choosen to get the correct scaled size
        sizeRanges[0]=particleRadius/5.0*1.0;
        sizeRanges[1]=particleRadius/5.0*2.0;
        sizeRanges[2]=particleRadius/5.0*4.0;
        sizeRanges[3]=particleRadius/5.0*6.0;
        sizeRanges[4]=particleRadius/5.0*8.0;
        sizeRanges[5]=particleRadius/5.0*10.0;
...
int main(int argc UNUSED, char *argv[] UNUSED)
{
    ...
    //
    Mdouble tc = 2e-4; //calculated as tc = 0.05-0.1 * tg : tg=sqrt(d50/g)
    ...
    //
    Mdouble SlidingFrictionCoeff = 0.5;//1.0/studyNum[1];//0.5;
    Mdouble RollingFrictionCoeff = 0.1;//2.0/(studyNum[2]*10.0);//0.1;
    ...
    //------------------------------------------------------------------------------------------------------------------------------------------------------------
    //
    deposition_problem.setName("Deposition_Ti_Calibration"); //Deposition_gaussian_parametric
