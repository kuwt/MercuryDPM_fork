...
int main()
{
    ...
    Mdouble radius = 30e-3; //each particle has 5g
    ...
    unsigned nDoms = 1; //the number of domains used for parallelisation
    ...
    //setup simulation
    CSK csk(&particleSpecies, &particleWallSpecies, fillVolume, outflowRate, rpm, radius,false,true,false);
    //Set the number of domains for parallel decomposition
    csk.setNumberOfDomains({nDoms,1,1});
    std::stringstream ss;
	std::string baseName = "phase3_mix";
	ss << baseName << "-r" << radius << "-rpm" << rpm << "-domains" << nDoms;
	csk.setName(ss.str());
    csk.setTimeStep(0.02 * collisionTime); 
    csk.setSaveCount(200.0*collisionTime/csk.getTimeStep()); //normally 500* -- save every 500 collisiions
    csk.setTimeMax(36.0*600.0/rpm);
    //for quick testing
    //csk.setTimeMax(1.0);
