...
def resampledParamsTable(keys,smcSamples,proposal,num=100,thread=4,maxNumComponents=10,priorWeight=1e3,tableName='smcTableNew.txt',verbose=False):
	if (verbose):
		print("Called resampledParamsTable(keys=%s, smcSamples=%s, proposal=%s, num=%s, thread=%s, maxNumComponents=%s, priorWeight=%s, tableName=%s)"
			  % (keys,smcSamples,proposal,num,thread,maxNumComponents,priorWeight,tableName))
	dim = len(keys)
	...
	# print("BayesianGaussianMixture fit: means %s, weights %s, covs %s"
	#	  % (gmm.means_*sampleMaxs, gmm.weights_, gmm.covariances_))
	if verbose:
		print("Fitting smcNewSamples=%s with %s" % (smcNewSamples, gmm))
