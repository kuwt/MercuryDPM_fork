/*=========================================================================
  ...
  Module:    vtkTensorGlyphSameEigensystem.cxx
...
vtkStandardNewMacro(vtkTensorGlyphSameEigensystem);

vtkTensorGlyphSameEigensystem::vtkTensorGlyphSameEigensystem()
{
  this->HaveEigenvalues = 0;
}
...
//----------------------------------------------------------------------------
// This is a tweaked copy of vtkTensorGlyph::RequestData
int vtkTensorGlyphSameEigensystem::RequestData(
  ...
  vtkInformationVector *outputVector)
{
  ...
  for (inPtId=0; inPtId < numPts; inPtId++)
    ...
    if ( this->ExtractEigenvalues ) // extract appropriate eigenfunctions if required
      if ( this->HaveEigenvalues )
        {
        // Use stored eigenvalues and eigenvectors
        w[0] = this->Eigenvalues[0];
        w[1] = this->Eigenvalues[1];
        w[2] = this->Eigenvalues[2];
        xv[0] = this->EigenvectorsX[0]; xv[1] = this->EigenvectorsX[1]; xv[2] = this->EigenvectorsX[2];
        yv[0] = this->EigenvectorsY[0]; yv[1] = this->EigenvectorsY[1]; yv[2] = this->EigenvectorsY[2];
        zv[0] = this->EigenvectorsZ[0]; zv[1] = this->EigenvectorsZ[1]; zv[2] = this->EigenvectorsZ[2];
        }
      else
        {
        for (j=0; j<3; j++)
        ...
        // store results for use with all other points
        SetEigenvectorsEigenValues(v, w);
        }
...
// //----------------------------------------------------------------------------
// // Don't rely on the implementation of vtkTensorGlyph::RequestUpdateExtent
// // as that has, at point of writing, a known issue. See issue 3 at
// // http://www.paraview.org/Bug/view.php?id=12179
// int vtkTensorGlyphSameEigensystem::RequestUpdateExtent(
//   vtkInformation *vtkNotUsed(request),
//   vtkInformationVector **inputVector,
//   vtkInformationVector *outputVector)
// {
...
void vtkTensorGlyphSameEigensystem::SetEigenvectorsEigenValues(double **ev, double *ew)
{
  if (ev && ew)
    {
    this->Eigenvalues[0] = ew[0];
    this->Eigenvalues[1] = ew[1];
    this->Eigenvalues[2] = ew[2];

    this->EigenvectorsX[0] = ev[0][0]; this->EigenvectorsX[1] = ev[1][0]; this->EigenvectorsX[2] = ev[2][0];
    this->EigenvectorsY[0] = ev[0][1]; this->EigenvectorsY[1] = ev[1][1]; this->EigenvectorsY[2] = ev[2][1];
    this->EigenvectorsZ[0] = ev[0][2]; this->EigenvectorsZ[1] = ev[1][2]; this->EigenvectorsZ[2] = ev[2][2];

    this->HaveEigenvalues = 1;
    }
