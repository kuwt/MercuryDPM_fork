...
#include "AndersonJacksonElements.h"
...
/// Anderson-Jackson equations static data
template<unsigned DIM>
Vector<double> AndersonJacksonEquations<DIM>::Gamma(DIM,1.0);
...
//=================================================================
template<unsigned DIM>
int AndersonJacksonEquations<DIM>::Pressure_not_stored_at_node = -100;

/// Navier--Stokes equations static data
template<unsigned DIM>
double AndersonJacksonEquations<DIM>::Default_Physical_Constant_Value = 0.0;

/// Navier--Stokes equations static data
template<unsigned DIM>
double AndersonJacksonEquations<DIM>::Default_Physical_Ratio_Value = 1.0;

/// Navier-Stokes equations default gravity vector
template<unsigned DIM>
Vector<double> AndersonJacksonEquations<DIM>::Default_Gravity_vector(DIM,0.0);
...
//===================================================================
 template<unsigned DIM>
 void AndersonJacksonEquations<DIM>::
...
//=======================================================================
template <unsigned DIM>
void AndersonJacksonEquations<DIM>::compute_norm(double& norm)
{
...
//=======================================================================
template<unsigned DIM>
void AndersonJacksonEquations<DIM>::compute_error(
 std::ostream &outfile,
...
//=======================================================================
template<unsigned DIM>
void AndersonJacksonEquations<DIM>::output_fct(std::ostream &outfile,
...
//==============================================================
template<unsigned DIM>
void AndersonJacksonEquations<DIM>::output_veloc(std::ostream& outfile,
                                              const unsigned& nplot,
                                              const unsigned& t)
{
...
//==============================================================
template<unsigned DIM>
void AndersonJacksonEquations<DIM>::output(std::ostream &outfile,
                                        const unsigned &nplot)
{
 ...
 for (unsigned iplot=0;iplot<num_plot_points;iplot++)
  {
   ...
   //FIXME Add voidage here Sept 2019
...
//==============================================================
template<unsigned DIM>
void AndersonJacksonEquations<DIM>::output(FILE* file_pt,
                                        const unsigned &nplot)
{
...
//==============================================================
template<unsigned DIM>
void AndersonJacksonEquations<DIM>::full_output(std::ostream &outfile,
                                             const unsigned &nplot)
{
...
///----------------------------------- Added by Mitchel -----------------------------------------
////==============================================================
///// Output function for voidage.
///// x,y,[z],voidage,dvoidagedx,dvoidagedy,[and/or dvoidagedz]]
///// in tecplot format. Specified number of plot points in each
///// coordinate direction.
////==============================================================
template<unsigned DIM>
void AndersonJacksonEquations<DIM>::output_voidage(std::ofstream &outfile, const unsigned &nplot)
{
    /// Rewritten at 13 Sept 2019 to make it work for coupling on element basis rather than itp.
    /// Also went from tecplot format to own defined format
    ...
    double currTime = time_stepper_pt->time();
    
    unsigned num_plot_points=nplot_points(nplot);
    for (unsigned iplot=0;iplot<num_plot_points;iplot++)
    {
        get_s_plot(iplot,nplot,s);
        oomph::Vector<double> currPos(DIM,0.0);
        for(unsigned i=0;i<DIM;i++)
        {
            currPos[i] = interpolated_x(s,i);
            outfile << interpolated_x(s,i) << " ";
        }
        
        outfile << voidage_nst(currPos) << " ";
        
        oomph::Vector<double> dvoidagedx(DIM,0.0);
        dvoidagedx_nst(currPos,dvoidagedx);
        for (unsigned int i=0; i<DIM; i++)
        {
            outfile << dvoidagedx[i] << " ";
        }
        
        double dvoidagedt=0.0;
        dvoidagedt_nst(currPos,dvoidagedt);
        outfile << dvoidagedt << " ";
...
////==============================================================
template<unsigned DIM>
void AndersonJacksonEquations<DIM>::output_voidage_byEl(std::ofstream &outfile, const unsigned &nplot, const unsigned int &iEl)
{
    oomph::Vector<double> s(DIM);
    int elNr = iEl;
    ...
    for (unsigned iplot=0;iplot<num_plot_points;iplot++)
    {
        ...
        outfile << voidage_nst_byEl(elNr) << " ";

        oomph::Vector<double> dvoidagedx(DIM,0.0);
        dvoidagedx_nst_byEl(elNr, dvoidagedx);
        ...
        double dvoidagedt=0.0;
        dvoidagedt_nst_byEl(elNr,dvoidagedt);
...
//==============================================================
template<unsigned DIM>
void AndersonJacksonEquations<DIM>::output_vorticity(std::ostream &outfile,
                                                  const unsigned &nplot)
{
...
//==============================================================
template<unsigned DIM>
double AndersonJacksonEquations<DIM>::dissipation() const
{
...
//==============================================================
template<unsigned DIM>
 void  AndersonJacksonEquations<DIM>::get_traction(const Vector<double>& s,
...
//==============================================================
template<unsigned DIM>
double AndersonJacksonEquations<DIM>::dissipation(const Vector<double>& s) const
{
 // Get strain rate matrix
 DenseMatrix<double> strainrate(DIM,DIM);
 strain_rate(s,strainrate);
...
//==============================================================
template<unsigned DIM>
void AndersonJacksonEquations<DIM>::strain_rate(const Vector<double>& s,
...
//==============================================================
template<>
void AndersonJacksonEquations<2>::get_vorticity(const Vector<double>& s,
...
//==============================================================
template<>
void AndersonJacksonEquations<3>::get_vorticity(const Vector<double>& s,
                                             Vector<double>& vorticity) const
{
...
//==============================================================
template<unsigned DIM>
double AndersonJacksonEquations<DIM>::kin_energy() const
{
...
//==========================================================================
template<unsigned DIM>
double AndersonJacksonEquations<DIM>::d_kin_energy_dt() const
{
 // Initialise
...
//==============================================================
template<unsigned DIM>
double AndersonJacksonEquations<DIM>::pressure_integral() const
{
...
///----------------------------------- Added by Mitchel 11 Sept 2019 -----------------------------------------
    int elNr = this->Number;
    double void_ratio = this->get_voidage_nst_byEl(elNr);
    oomph::Vector<double> dvoidagedx(DIM,0.0);
    this->get_voidage_gradient_nst_byEl(elNr, dvoidagedx);
    double dvoidagedt;
...
//    std::cout << "dvoidagedx = " << dvoidagedx[0] << ", " << dvoidagedx[1] << ", " << dvoidagedx[2] << std::endl;
//    std::cout << "dvoidagedt = " << dvoidagedt << std::endl;
//    std::cout << "bodyForceByCoupling = " << bodyForceByCoupling[0] << ", " << bodyForceByCoupling[1] << ", " << bodyForceByCoupling[2] << std::endl << std::endl;
...
///----------------------------------- Added by Mitchel -----------------------------------------
         residuals[local_eqn] += void_ratio*body_force[i]*testf[l]*W;

         //Add the gravitational body force term
         residuals[local_eqn] += void_ratio*scaled_re_inv_fr*testf[l]*G[i]*W;
         
         /// Add the body force due to pf-coupling
         residuals[local_eqn] += bodyForceByCoupling[i]*testf[l]*W;
    
           std::cout << "bodyForceByCoupling[" << i << "] = " <<  bodyForceByCoupling[i] << std::endl;
           std::cout << "bodyForceByCoupling[" << i << "]*testf[" << l << "]*W = " <<  bodyForceByCoupling[i]*testf[l]*W << std::endl;
///----------------------------------- Added void_ratio* ----------------------------------------
         ...
         for(unsigned k=0;k<DIM;k++)
          {
           residuals[local_eqn] -= visc_ratio * (interpolated_dudx(i,k) + Gamma[i]*interpolated_dudx(k,i)) * dtestfdx(l,k)*W;
          }
         
///----------------------------------- Added by Mitchel -----------------------------------------
         //Add in the inertial terms
         //du/dt term
         residuals[local_eqn] -= void_ratio*scaled_re_st*dudt[i]*testf[l]*W;
         ...
         for(unsigned k=0;k<DIM;k++)
          {
           ...
           residuals[local_eqn] -= void_ratio*tmp*interpolated_dudx(i,k)*testf[l]*W;

          }
///----------------------------------- Added void_ratio* ----------------------------------------

        //fixme There is a term missing here due to the particle fluid coupling term taht is added as body force term
               ...
               if(local_unknown >= 0)
                {
                 ...
                 jacobian(local_eqn,local_unknown) -= visc_ratio*Gamma[i]*dpsifdx(l2,i)*dtestfdx(l,i2)*W;
                 ...
                 if(i2 == i)
                  {
                   ...
                   for(unsigned k=0;k<DIM;k++)
                    {
                     jacobian(local_eqn,local_unknown) -= visc_ratio*dpsifdx(l2,k)*dtestfdx(l,k)*W;
                    }
                  }
///----------------------------------- Added by Mitchel -----------------------------------------
                 //Now add in the inertial terms
                 jacobian(local_eqn,local_unknown) -= void_ratio*scaled_re*psif[l2]*interpolated_dudx(i,i2)*testf[l]*W;
///----------------------------------- Added void_ratio* -----------------------------------------
...
///----------------------------------- Added by Mitchel -----------------------------------------
                   if(flag==2)
                    {
                     mass_matrix(local_eqn,local_unknown) += void_ratio*scaled_re_st*psif[l2]*testf[l]*W;
                    }

                   //du/dt term
                   jacobian(local_eqn,local_unknown) -= void_ratio*scaled_re_st *
                           node_pt(l2)->time_stepper_pt()->weight(1,0) * psif[l2] * testf[l] * W;
                   ...
                   for(unsigned k=0;k<DIM;k++)
                    {
                     double tmp=scaled_re*interpolated_u[k];
                     if (!ALE_is_disabled) tmp-=scaled_re_st*mesh_velocity[k];
                     jacobian(local_eqn,local_unknown) -= void_ratio*tmp*dpsifdx(l2,k)*testf[l]*W;
                    }
///----------------------------------- Added void_ratio* ----------------------------------------
                  }
                }
       ...
       for(unsigned k=0;k<DIM;k++)
        {
         ...
         aux += void_ratio*interpolated_dudx(k,k);
         aux += interpolated_u[k]*dvoidagedx[k];
        }
 ///----------------------------------- Added void_ratio* ----------------------------------------
...
//======================================================================
template <unsigned DIM>
void AndersonJacksonEquations<DIM>::get_dresidual_dnodal_coordinates(
...
///----------------------------------- Added by Mitchel 11 Sept 2019 -----------------------------------------
 ...
 ///----------------------------------- Added by Mitchel -----------------------------------------
             //Add the user-defined body force terms
             double sum = void_ratio*body_force[i]*testf[l];

             //Add the gravitational body force term
             sum += void_ratio*scaled_re_inv_fr*testf[l]*G[i];
///----------------------------------- Added void_ratio* ------------------------------------------
             ...
             // du/dt term
///----------------------------------- Added by Mitchel -----------------------------------------
             sum -= void_ratio*scaled_re_st*dudt[i]*testf[l];
             ...
             for(unsigned k=0;k<DIM;k++)
              {
               double tmp=scaled_re*interpolated_u[k];
               if (!ALE_is_disabled) { tmp-=scaled_re_st*mesh_velocity[k]; }
               sum -= void_ratio*tmp*interpolated_dudx(i,k)*testf[l];
              }
///----------------------------------- Added void_ratio* ------------------------------------------
...
///----------------------------------- Added by Mitchel -----------------------------------------
             // Body force
             sum=void_ratio*d_body_force_dx(i,p)*psif(q)*testf(l);
///----------------------------------- Added void_ratio* ------------------------------------------
             ...
             for(unsigned k=0;k<DIM;k++)
              {
               double tmp=scaled_re*interpolated_u[k];
               if(!ALE_is_disabled) { tmp-=scaled_re_st*mesh_velocity[k]; }
               sum -= void_ratio*tmp*d_dudx_dX(p,q,i,k)*testf(l);
              }
              if(!ALE_is_disabled)
               {
                sum+=void_ratio*scaled_re_st*pos_time_weight*
             ...
             if(element_has_node_with_aux_node_update_fct)
              {
               sum=-visc_ratio*Gamma[i]*dpsifdx(q,i)*dtestfdx(l,p)
                -void_ratio*scaled_re*psif(q)*interpolated_dudx(i,p)*testf(l);
               if (i==p)
                {
                 sum-=void_ratio*scaled_re_st*val_time_weight*psif(q)*testf(l);
                 for (unsigned k=0;k<DIM;k++)
                  {
                   sum-=visc_ratio*dpsifdx(q,k)*dtestfdx(l,k);
                   double tmp=scaled_re*interpolated_u[k];
                   if (!ALE_is_disabled) tmp-=scaled_re_st*mesh_velocity[k];
                   sum-=void_ratio*tmp*dpsifdx(q,k)*testf(l);
                  }
                }
...
///----------------------------------- Added by Mitchel -----------------------------------------
           for(unsigned k=0;k<DIM;k++)
            {
             aux += void_ratio*d_dudx_dX(p,q,k,k);
            }
///----------------------------------- Added void_ratio* ------------------------------------------
...
template<>
const unsigned AJQCrouzeixRaviartElement<2>::Initial_Nvalue[9]={2,2,2,2,2,2,2,2,2};

///3D Crouzeix-Raviart elements
//Set the data for the number of Variables at each node
template<>
const unsigned AJQCrouzeixRaviartElement<3>::Initial_Nvalue[27]={3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3};

//========================================================================
/// Number of values (pinned or dofs) required at node n.
//========================================================================
template<unsigned DIM>
unsigned AJQCrouzeixRaviartElement<DIM>::required_nvalue(const unsigned& n) const
...
//=========================================================================
template<unsigned DIM>
void AJQCrouzeixRaviartElement<DIM>::
...
//=============================================================================
template<unsigned DIM>
void AJQCrouzeixRaviartElement<DIM>::get_dof_numbers_for_unknowns(
...
//Set the data for the number of Variables at each node
template<>
const unsigned AJQTaylorHoodElement<2>::Initial_Nvalue[9]={3,2,3,2,2,2,3,2,3}; /// What is this initialising? Nr of dofs per node in element

//Set the data for the pressure conversion array
template<>
const unsigned AJQTaylorHoodElement<2>::Pconv[4]={0,2,6,8};

//3D Taylor--Hood
//Set the data for the number of Variables at each node
template<>
const unsigned AJQTaylorHoodElement<3>::Initial_Nvalue[27]={4,3,4,3,3,3,4,3,4,3,3,3,3,3,3,3,3,3,4,3,4,3,3,3,4,3,4};

//Set the data for the pressure conversion array
template<>
const unsigned AJQTaylorHoodElement<3>::Pconv[8]={0,2,6,8,18,20,24,26};
...
//============================================================================
template<unsigned DIM>
void AJQTaylorHoodElement<DIM>::get_dof_numbers_for_unknowns(
...
//====================================================================
template class AndersonJacksonEquations<2>;
template class AJQCrouzeixRaviartElement<2>;
template class AJQTaylorHoodElement<2>;

template class AndersonJacksonEquations<3>;
template class AJQCrouzeixRaviartElement<3>;
template class AJQTaylorHoodElement<3>;
