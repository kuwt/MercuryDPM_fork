...
#ifndef OOMPH_ANDERSON_JACKSON_ELEMENTS_HEADER
#define OOMPH_ANDERSON_JACKSON_ELEMENTS_HEADER
...
//OOMPH-LIB headers
#include "../../../../../oomph-lib/src/generic/Qelements.h"
...
/// casting problems
//======================================================================
class TemplateFreeAndersonJacksonEquationsBase :
 ...
 public virtual FiniteElement
{
 ...
 /// Constructor (empty)
 TemplateFreeAndersonJacksonEquationsBase(){};

 /// Virtual destructor (empty)
 virtual ~TemplateFreeAndersonJacksonEquationsBase(){};
...
template <unsigned DIM>
 class AndersonJacksonEquations : public virtual FSIFluidElement,
 public virtual TemplateFreeAndersonJacksonEquationsBase
{
 ...
 /// x is a Vector!
 typedef void (*AndersonJacksonBodyForceFctPt)(const double& time,
                                            const Vector<double>& x,
                                            Vector<double>& body_force);

///----------------------------------- Added by Mitchel -----------------------------------------
     /// \short Function pointer to voidage function fct(t,x)
     typedef double (*AndersonJacksonVoidageFctPt)(const double& time, const oomph::Vector<double>& x);
    
     /// \short Function pointer to voidage function fct(elNr)
     typedef double (*AndersonJacksonVoidageFctPt_byEl)(const int& elNr);
    
     /// \short Function pointer to dvoidagedx function fct(elNr)
     typedef double (*AndersonJacksondVoidage_dx_fct_pt_byEl)(const int& elNr, const int& dir);
...
///----------------------------------- Added by Mitchel -----------------------------------------
 ...
 /// x is a Vector!
 typedef double (*AndersonJacksonSourceFctPt)(const double& time,
                                           const Vector<double>& x);
 ...
 typedef double (*AndersonJacksonPressureAdvDiffSourceFctPt)(
  const Vector<double>& x);
 ...
 AndersonJacksonBodyForceFctPt Body_force_fct_pt;

 ///----------------------------------- Added by Mitchel -----------------------------------------
     /// Pointer to global voidage
    AndersonJacksonVoidageFctPt Voidage_fct_pt;
 
    /// Pointer to voidage
    AndersonJacksonVoidageFctPt_byEl Voidage_fct_pt_byEl;
    
    /// Pointer to dvoidagedx
    AndersonJacksondVoidage_dx_fct_pt_byEl dVoidage_dx_fct_pt_byEl;
    
    /// Pointer to dvoidagedt
    AndersonJacksondVoidage_dt_fct_pt_byEl dVoidage_dt_fct_pt_byEl;
    
    /// Pointer to coupling forces (implemented as body force)
    AndersonJacksonBodyForceByCoupling_fct_pt_byEl BodyForceByCoupling_fct_pt_byEl;
 /// ---------------------------------------------------------------------------------------------

 /// Pointer to volumetric source function
 AndersonJacksonSourceFctPt Source_fct_pt;

 /// \short Pointer to source function pressure advection diffusion equation
 /// (only to be used during validation)
 AndersonJacksonPressureAdvDiffSourceFctPt Press_adv_diff_source_fct_pt;
...
///----------------------------------- Added by Mitchel -----------------------------------------
 /// \short Calculate the voidage fct at given time and Eul. pos.
 virtual double get_voidage_nst(const double& time, const unsigned& ipt, const oomph::Vector<double> &x)
 {
    //If the function pointer is zero return one (as voidage=1 means full fluid)
    if (Voidage_fct_pt == 0)
    {
        return 1.0;
    }
    ...
    {
        return (*Voidage_fct_pt)(time,x);
    }
 }

 virtual void get_voidage_gradient_nst(const double& time, const unsigned& ipt, const oomph::Vector<double> &x, oomph::Vector<double>& gradient)
 {
    double voidage = get_voidage_nst(time, ipt, x);
    double eps_fd = GeneralisedElement::Default_fd_jacobian_step;
    double voidage_pls;
    oomph::Vector<double> x_pls(x);
    for (unsigned i=0; i<DIM; i++)
    {
        x_pls[i] += eps_fd;
        voidage_pls = get_voidage_nst(time,ipt,x_pls);
        gradient[i]=(voidage_pls-voidage)/eps_fd;
        x_pls[i] = x[i];
    }
 }

 virtual void get_voidage_gradient_time_nst(const double& time, const unsigned& ipt, const oomph::Vector<double> &x, double& dedt)
 {
    double voidage = get_voidage_nst(time, ipt, x);
    double eps_fd = GeneralisedElement::Default_fd_jacobian_step;
    double voidage_t1;
    ...
    voidage_t1 = get_voidage_nst(t_pls,ipt,x);
    dedt=(voidage_t1-voidage)/eps_fd;
 }
    
    /// \short Calculate the voidage fct at given time and Eul. pos. based on elNr
    virtual double get_voidage_nst_byEl(const int& elNr)
    {
        //If the function pointer is zero return one (as voidage=1 means full fluid)
        if (Voidage_fct_pt_byEl == 0)
        {
            return 1.0;
        }
            //Otherwise call the function
    ...
    //FIXME gradients are undefined right now as element number doesnt store direction
    virtual void get_voidage_gradient_nst_byEl(const int& elNr, oomph::Vector<double>& gradient)
    {
        if (dVoidage_dx_fct_pt_byEl == 0)
        {
            for (unsigned i =0; i<DIM ; i++)
            {
                gradient[i] = 0;
        ...
        {
            for (unsigned iDir =0; iDir<DIM ; iDir++)
            {
                gradient[iDir] = (*dVoidage_dx_fct_pt_byEl)(elNr,iDir);
            }
        }
    }
    
    virtual void get_voidage_gradient_time_nst_byEl(const int& elNr, double& dedt)
    {
        if (dVoidage_dt_fct_pt_byEl == 0)
        {
            dedt = 0.0;
        }
        else
        {
            dedt = (*dVoidage_dt_fct_pt_byEl)(elNr);
        }
    }
...
public:
 ...
 AndersonJacksonEquations() : Body_force_fct_pt(0), Voidage_fct_pt(0), Voidage_fct_pt_byEl(0), dVoidage_dx_fct_pt_byEl(0),
  dVoidage_dt_fct_pt_byEl(0), BodyForceByCoupling_fct_pt_byEl(0), Source_fct_pt(0), Press_adv_diff_source_fct_pt(0), ALE_is_disabled(false),
 ...
 ///----------------------------------- Added by Mitchel -----------------------------------------
    ...
    AndersonJacksonVoidageFctPt voidage_fct_pt() const {return Voidage_fct_pt;}
    
    // Defined by elNr
    AndersonJacksonVoidageFctPt_byEl& voidage_fct_pt_byEl() {return Voidage_fct_pt_byEl;}
    AndersonJacksonVoidageFctPt_byEl voidage_fct_pt_byEl() const {return Voidage_fct_pt_byEl;}
    
    // Defined by elNr
    AndersonJacksondVoidage_dx_fct_pt_byEl& dvoidage_dx_fct_pt_byEl() {return dVoidage_dx_fct_pt_byEl;}
    AndersonJacksondVoidage_dx_fct_pt_byEl dvoidage_dx_fct_pt_byEl() const {return dVoidage_dx_fct_pt_byEl;}
    
    // Defined by elNr
    AndersonJacksondVoidage_dt_fct_pt_byEl& dvoidage_dt_fct_pt_byEl() {return dVoidage_dt_fct_pt_byEl;}
    AndersonJacksondVoidage_dt_fct_pt_byEl dvoidage_dt_fct_pt_byEl() const {return dVoidage_dt_fct_pt_byEl;}
    
    // Defined by elNr
    AndersonJacksonBodyForceByCoupling_fct_pt_byEl& body_force_fct_pt_by_coupling() {return BodyForceByCoupling_fct_pt_byEl;}
    AndersonJacksonBodyForceByCoupling_fct_pt_byEl body_force_fct_pt_by_coupling() const {return BodyForceByCoupling_fct_pt_byEl;}
    ...
    void setNumber(const long& newNr_) {Number = newNr_;}
 ///----------------------------------------------------------------------------------------------

 ///Access function for the source-function pointer
 AndersonJacksonSourceFctPt& source_fct_pt() {return Source_fct_pt;}

 ///Access function for the source-function pointer. Const version
 AndersonJacksonSourceFctPt source_fct_pt() const {return Source_fct_pt;}

 /// \short Access function for the source-function pointer for pressure
 /// advection diffusion  (used for validation only).
 AndersonJacksonPressureAdvDiffSourceFctPt& source_fct_for_pressure_adv_diff()
...
///----------------------------------- Added by Mitchel -----------------------------------------
    double voidage_nst(oomph::Vector<double> &x) const
    {
        TimeStepper* time_stepper_pt = this->node_pt(0)->time_stepper_pt();
        double currTime = time_stepper_pt->time();

        return (*Voidage_fct_pt)(currTime,x);
    }

    void dvoidagedx_nst(oomph::Vector<double> &x, oomph::Vector<double> &dvoidagedx) const
    {
        double eps = GeneralisedElement::Default_fd_jacobian_step;
        TimeStepper* time_stepper_pt = this->node_pt(0)->time_stepper_pt();
        double currTime = time_stepper_pt->time();
        ...
        {
            x_pls[i] += eps;
            double voidAtx = (*Voidage_fct_pt)(currTime,x);
            double voidAtxdx = (*Voidage_fct_pt)(currTime,x_pls);
            dvoidagedx[i] = (voidAtxdx - voidAtx) /eps;
            x_pls[i]-= eps;
        }
    }

    void dvoidagedt_nst(oomph::Vector<double> &x, double &dvoidagedt) const
    {
        double eps = GeneralisedElement::Default_fd_jacobian_step;
        TimeStepper* time_stepper_pt = this->node_pt(0)->time_stepper_pt();
        double currTime = time_stepper_pt->time();
        double t_pls = currTime;
        t_pls += eps;

        double voidAtt = (*Voidage_fct_pt)(currTime,x);
        double voidAttdt = (*Voidage_fct_pt)(t_pls,x);
        dvoidagedt = (voidAttdt - voidAtt) /eps;
    }
    
    // By elNr
    double voidage_nst_byEl(int &elNr) const
    {
        return (*Voidage_fct_pt_byEl)(elNr);
    }
    
    void dvoidagedx_nst_byEl(int &elNr, oomph::Vector<double> &dvoidagedx) const
    {
        for (unsigned int iDir = 0; iDir < DIM; iDir++)
        {
            dvoidagedx[iDir] = (*dVoidage_dx_fct_pt_byEl)(elNr, iDir);
        }
    }
    
    void dvoidagedt_nst_byEl(int &elNr, double &dvoidagedt) const
    {
        
        dvoidagedt = (*dVoidage_dt_fct_pt_byEl)(elNr);
    }
...
///----------------------------------- Added by Mitchel -----------------------------------------
    virtual void output_voidage(std::ofstream &outfile)
    {
        unsigned nplot = 5;
        output_voidage(outfile,nplot);
    }
    virtual void output_voidage(std::ofstream &outfile, const unsigned &nplot);
    
    virtual void output_voidage_byEl(std::ofstream &outfile, const unsigned int &iEl)
    {
        unsigned nplot = 5;
        output_voidage_byEl(outfile,nplot,iEl);
    }
    virtual void output_voidage_byEl(std::ofstream &outfile, const unsigned &nplot, const unsigned int &iEl);
///----------------------------------- Added by Mitchel -----------------------------------------
                                ...
                                eqn_number_backup)
 {
  ...
  unsigned nint=this->ninternal_data(); //FIXME this might need change when voidage is added as internal data
                              ...
                              const unsigned &j) const
 {
  ...
  // with respect to space
   ...
   return(interpolated_dudx);
    ...
    ///----------------------------------- Added by Mitchel -----------------------------------------
    double interpolated_dpdx_nst(const oomph::Vector<double> &s, const unsigned &i) const
    {
        //std::cout << "In interpolated_dpdx_nst" << std::endl;
        const unsigned n_pres = npres_nst();
        Shape psip(n_pres);
        DShape dpsipdx(n_pres,DIM);
        Shape psiptest(n_pres);
        DShape dpsiptestdx(n_pres,DIM);
        
        //std::cout << "\t allocation ok, n_pres = " << n_pres << std::endl;
        
        (void)this->dpshape_and_dptest_eulerian_nst(s,psip,dpsipdx,psiptest,dpsiptestdx); //FIXME here the code crashes
        const unsigned p_nodal_index = p_nodal_index_nst();
    
        double interpolated_dpdx = 0.0;
        ...
        for(unsigned l=0;l<n_pres;l++)
        {
            interpolated_dpdx += p_nst(l) * dpsipdx(l,i);
            //interpolated_dpdx += nodal_value(l,p_nodal_index)*dpsipdx(l,i);
        }
...
//==========================================================================
template <unsigned DIM>
class AJQCrouzeixRaviartElement : public virtual QElement<DIM,3>,
 public virtual AndersonJacksonEquations<DIM>
{
  private:
...
public:
 ...
 AJQCrouzeixRaviartElement() : QElement<DIM,3>(), AndersonJacksonEquations<DIM>()
  {
                                               ...
                                               face_index)
 {
  this->Pressure_advection_diffusion_robin_element_pt.push_back(
   new FpPressureAdvDiffRobinBCElement<AJQCrouzeixRaviartElement<DIM> >(
    this, face_index));
 }
 ...
 void identify_pressure_data(
  std::set<std::pair<Data*,unsigned> > &paired_pressure_data);
    ...
    /// Redirect output to AndersonJacksonEquations output
 void output(std::ostream &outfile)
  {AndersonJacksonEquations<DIM>::output(outfile);}

 /// Redirect output to AndersonJacksonEquations output
 void output(std::ostream &outfile, const unsigned &nplot)
  {AndersonJacksonEquations<DIM>::output(outfile,nplot);}


 /// Redirect output to AndersonJacksonEquations output
 void output(FILE* file_pt) {AndersonJacksonEquations<DIM>::output(file_pt);}

 /// Redirect output to AndersonJacksonEquations output
 void output(FILE* file_pt, const unsigned &nplot)
  {AndersonJacksonEquations<DIM>::output(file_pt,nplot);}
 ...
 /// in tecplot format. Default number of plot points
 void full_output(std::ostream &outfile)
  {AndersonJacksonEquations<DIM>::full_output(outfile);}

 /// \short Full output function:
 /// x,y,[z],u,v,[w],p,du/dt,dv/dt,[dw/dt],dissipation
 /// in tecplot format. nplot points in each coordinate direction
 void full_output(std::ostream &outfile, const unsigned &nplot)
  {AndersonJacksonEquations<DIM>::full_output(outfile,nplot);}

///----------------------------------- Added by Mitchel -----------------------------------------
    virtual void output_voidage(std::ofstream &outfile)
    {AndersonJacksonEquations<DIM>::output_voidage(outfile);}
    
    virtual void output_voidage(std::ofstream &outfile, const unsigned &nplot)
    {AndersonJacksonEquations<DIM>::output_voidage(outfile,nplot);}
    
    virtual void output_voidage_byEl(std::ofstream &outfile, const unsigned int &iEl)
    {AndersonJacksonEquations<DIM>::output_voidage_byEl(outfile,iEl);}
    
    virtual void output_voidage_byEl(std::ofstream &outfile, const unsigned &nplot, const unsigned int &iEl)
    {AndersonJacksonEquations<DIM>::output_voidage_byEl(outfile,nplot,iEl);}
...
//=======================================================================
template<unsigned DIM>
inline double AJQCrouzeixRaviartElement<DIM>::dshape_and_dtest_eulerian_nst(
...
//=======================================================================
template<unsigned DIM>
inline double AJQCrouzeixRaviartElement<DIM>::
...
//=======================================================================
template<>
inline double AJQCrouzeixRaviartElement<2>::
dshape_and_dtest_eulerian_at_knot_nst(
...
//=======================================================================
template<>
inline double AJQCrouzeixRaviartElement<3>::
dshape_and_dtest_eulerian_at_knot_nst(
 const unsigned &ipt, Shape &psi, DShape &dpsidx,
 RankFourTensor<double> &d_dpsidx_dX,
 Shape &test, DShape &dtestdx,
...
//=======================================================================
template<>
inline void AJQCrouzeixRaviartElement<2>::pshape_nst(const Vector<double> &s,
                                                  Shape &psi)
const
{
 psi[0] = 1.0;
...
//==========================================================================
template<>
 inline double AJQCrouzeixRaviartElement<2>::dpshape_and_dptest_eulerian_nst(
  const Vector<double> &s,
  Shape &ppsi,
  DShape &dppsidx,
  Shape &ptest,
...
//=======================================================================
template<unsigned DIM>
 inline void AJQCrouzeixRaviartElement<DIM>::
...
//=======================================================================
template<>
inline void AJQCrouzeixRaviartElement<3>::pshape_nst(const Vector<double> &s,
                                                  Shape &psi)
const
{
 psi[0] = 1.0;
...
//==========================================================================
template<>
 inline double AJQCrouzeixRaviartElement<3>::dpshape_and_dptest_eulerian_nst(
  const Vector<double> &s,
  Shape &ppsi,
  DShape &dppsidx,
  Shape &ptest,
...
//=======================================================================
template<>
class FaceGeometry<AJQCrouzeixRaviartElement<2> >: public virtual QElement<1,3>
{
  public:
...
//=======================================================================
template<>
class FaceGeometry<AJQCrouzeixRaviartElement<3> >: public virtual QElement<2,3>
{
...
//=======================================================================
template<>
class FaceGeometry<FaceGeometry<AJQCrouzeixRaviartElement<2> > >:
...
//=======================================================================
template<>
class FaceGeometry<FaceGeometry<AJQCrouzeixRaviartElement<3> > >:
...
//=======================================================================
template <unsigned DIM>
class AJQTaylorHoodElement : public virtual QElement<DIM,3>,
 public virtual AndersonJacksonEquations<DIM>
{
  private:
 ...
 AJQTaylorHoodElement() : QElement<DIM,3>(),  AndersonJacksonEquations<DIM>() {}
 ...
 {
  this->Pressure_advection_diffusion_robin_element_pt.push_back(
   new FpPressureAdvDiffRobinBCElement<AJQTaylorHoodElement<DIM> >(
    this, face_index));
 }
...
//==========================================================================
template<unsigned DIM>
inline double AJQTaylorHoodElement<DIM>::dshape_and_dtest_eulerian_nst(
 const Vector<double> &s,
 Shape &psi,
 DShape &dpsidx, Shape &test,
 DShape &dtestdx) const
...
//==========================================================================
template<unsigned DIM>
inline double AJQTaylorHoodElement<DIM>::dshape_and_dtest_eulerian_at_knot_nst(
...
//==========================================================================
template<>
 inline double AJQTaylorHoodElement<2>::dpshape_and_dptest_eulerian_nst(
  const Vector<double> &s,
  Shape &ppsi,
  DShape &dppsidx,
  Shape &ptest,
...
//==========================================================================
template<>
inline double AJQTaylorHoodElement<2>::
 dshape_and_dtest_eulerian_at_knot_nst(
  const unsigned &ipt, Shape &psi, DShape &dpsidx,
  RankFourTensor<double> &d_dpsidx_dX,
  Shape &test, DShape &dtestdx,
...
//==========================================================================
template<>
 inline double AJQTaylorHoodElement<3>::
 dshape_and_dtest_eulerian_at_knot_nst(
  const unsigned &ipt, Shape &psi, DShape &dpsidx,
  RankFourTensor<double> &d_dpsidx_dX,
  Shape &test, DShape &dtestdx,
...
//==========================================================================
template<>
inline void AJQTaylorHoodElement<2>::pshape_nst(const Vector<double> &s,
                                              Shape &psi)
const
{
 //Local storage
...
//==========================================================================
template<>
 inline double AJQTaylorHoodElement<3>::dpshape_and_dptest_eulerian_nst(
  const Vector<double> &s,
  Shape &ppsi,
  DShape &dppsidx,
  Shape &ptest,
...
//==========================================================================
template<>
inline void AJQTaylorHoodElement<3>::pshape_nst(const Vector<double> &s,
                                              Shape &psi)
const
{
 //Local storage
...
//==========================================================================
template<unsigned DIM>
inline void AJQTaylorHoodElement<DIM>::pshape_nst(const Vector<double> &s,
                                                Shape &psi,
                                                Shape &test) const
{
 //Call the pressure shape functions
...
//=======================================================================
template<>
class FaceGeometry<AJQTaylorHoodElement<2> >: public virtual QElement<1,3>
{
  public:
 FaceGeometry() : QElement<1,3>() {}
};
...
//=======================================================================
template<>
class FaceGeometry<AJQTaylorHoodElement<3> >: public virtual QElement<2,3>
{
...
//=======================================================================
template<>
class FaceGeometry<FaceGeometry<AJQTaylorHoodElement<2> > >:
public virtual PointElement
{
  public:
 FaceGeometry() : PointElement() {}
...
//=======================================================================
template<>
class FaceGeometry<FaceGeometry<AJQTaylorHoodElement<3> > >:
public virtual QElement<1,3>
{
  public:
 FaceGeometry() : QElement<1,3>() {}
...
//==========================================================
 ...
 class ProjectableAJTaylorHoodElement :
 ...
 public:
  ...
  ProjectableAJTaylorHoodElement(){}
...
//=======================================================================
 template<class ELEMENT>
 class FaceGeometry<ProjectableAJTaylorHoodElement<ELEMENT> >
...
//=======================================================================
 template<class ELEMENT>
 class FaceGeometry<FaceGeometry<ProjectableAJTaylorHoodElement<ELEMENT> > >
...
//==========================================================
 ...
 class ProjectableAJCrouzeixRaviartElement :
 ...
 public:
  ...
  /// \short Constructor [this was only required explicitly
  /// from gcc 4.5.2 onwards...]
  ProjectableAJCrouzeixRaviartElement(){}
...
//=======================================================================
 template<class ELEMENT>
 class FaceGeometry<ProjectableAJCrouzeixRaviartElement<ELEMENT> >
  : public virtual FaceGeometry<ELEMENT>
 {
 public:
  FaceGeometry() : FaceGeometry<ELEMENT>() {}
...
//=======================================================================
 template<class ELEMENT>
 class FaceGeometry<FaceGeometry<ProjectableAJCrouzeixRaviartElement<ELEMENT> > >
  : public virtual FaceGeometry<FaceGeometry<ELEMENT> >
 {
 public:
  FaceGeometry() : FaceGeometry<FaceGeometry<ELEMENT> >() {}
