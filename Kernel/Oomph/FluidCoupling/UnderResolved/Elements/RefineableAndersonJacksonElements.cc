...
//LIC//
//LIC//====================================================================
 #include "RefineableAndersonJacksonElements.h"
...
//===================================================================
 ...
 void RefineableAndersonJacksonEquations<DIM>::
...
///----------------------------------- Added by Mitchel 11 Sept 2019 -----------------------------------------
    ...
    if (bodyForceByCoupling[2] > 1e-6)
    {
        ...
        std::cout << "bodyForceByCoupling = " << bodyForceByCoupling[0] << ", " << bodyForceByCoupling[1] << ", "
              << bodyForceByCoupling[2] << std::endl << std::endl;
    }
///----------------------------------- Added by Mitchel 11 Sept 2019 -----------------------------------------
...
///----------------------------------- Added by Mitchel 11 Sept 2019 ----------------------------------------- //FIXME work this out

/* // Get the user-defined voidage function
    double void_ratio = this->get_voidage_nst(time,ipt, interpolated_x);
    oomph::Vector<double> dvoidagedx(DIM,0.0);
    this->get_voidage_gradient_nst(time,ipt, interpolated_x, dvoidagedx);

    double dvoidagedt;
    this->get_voidage_gradient_time_nst(time,ipt,interpolated_x, dvoidagedt);

    // Obtain values by pretending we only know epsilon on the nodal positions:
    //void_ratio = 0;
    for (unsigned int i=0; i<DIM;i++){dvoidagedx[i]=0.0;}
    dvoidagedt = 0.0;
    ...
    for(unsigned l=0;l<n_node;l++)
    {
        ...
        for (unsigned i = 0; i < DIM; i++) {
            currPos[i] = this->nodal_position(l, i);
        }
        for (unsigned i = 0; i < DIM; i++) {
            dvoidagedx[i] += this->get_voidage_nst(time, ipt, currPos) * dpsifdx(l, i);
        }

        //void_ratio += this->epsilon_nst(l) * psif[l];
        
        this->get_voidage_gradient_time_nst(time, ipt, currPos, dvoidagedt);
        dvoidagedt *= psif[l];
    }
       ...
       if(local_eqn>= 0)
        {
         ...
         sum += void_ratio*body_force[i];
         
         /// Add the body force due to pf-coupling
         sum += bodyForceByCoupling[i];
         //residuals[local_eqn] += bodyForceByCoupling[i];
         
//         if (bodyForceByCoupling[i] != 0.)
//         {
//             std::cout << "bodyForceByCoupling[" << i << "] = " <<  bodyForceByCoupling[i] << std::endl;
//         }
         
         //Add the gravitational body force term
         sum += void_ratio*scaled_re_inv_fr*G[i];

         //Add in the inertial term
         sum -= void_ratio*scaled_re_st*dudt[i];
         ...
         for(unsigned k=0;k<DIM;k++)
          {
           ...
           sum -= void_ratio*tmp*interpolated_dudx(i,k);
          }
         ///----------------------------------- Added void_ratio* ----------------------------------------
//            if (sum != 0.)
//            {
...
//            if (sum != 0.)
//            {
//                std::cout << "residuals[local_eqn] = " <<  sum << std::endl << std::endl;
//            }
...
///----------------------------------- Added by Mitchel -----------------------------------------
                   jacobian(local_eqn,local_unknown) -= void_ratio*scaled_re*psif[l2]*interpolated_dudx(i,i2)
                     ...
                     if(flag==2)
                      {
                       mass_matrix(local_eqn,local_unknown) +=
                        void_ratio*scaled_re_st*psif[l2]*testf[l]*W*
                     ...
                     for(unsigned k=0;k<DIM;k++)
                      {
                       ...
                       jacobian(local_eqn,local_unknown) -= void_ratio*tmp*dpsifdx(l2,k)*testf[l]*W*hang_weight*hang_weight2;
                      }
///----------------------------------- Added void_ratio* ----------------------------------------
                     ...
                     for(unsigned k=0;k<DIM;k++)
                      {
                       jacobian(local_eqn,local_unknown) -= visc_ratio*dpsifdx(l2,k)*dtestfdx(l,k)*W*hang_weight*hang_weight2;
                      }
             ...
             for(unsigned m2=0;m2<n_master2;m2++)
              {
               ...
               if(pressure_dof_is_hanging[l2])
                {
                 //Get the unknown from the master node
                 local_unknown = this->local_hang_eqn(hang_info2_pt->master_node_pt(m2),p_index);
                 //Get the weight from the hanging object
                 hang_weight2 = hang_info2_pt->master_weight(m2);
                }
               else
               ...
               if(local_unknown >= 0)
                {
                 jacobian(local_eqn,local_unknown) += psip[l2]*dtestfdx(l,i)*W*hang_weight*hang_weight2;
                }
              }
            }
     ...
     if(local_eqn >= 0)
      {
       ...
       residuals[local_eqn] -= source*testp[l]*W*hang_weight;
///----------------------------------- Added by Mitchel -----------------------------------------
       residuals[local_eqn] += dvoidagedt*testp[l]*W*hang_weight;
///----------------------------------- Added by Mitchel -----------------------------------------
       ...
       for(unsigned k=0;k<DIM;k++)
        {
         ///----------------------------------- Added by Mitchel -----------------------------------------
         residuals[local_eqn] += void_ratio*interpolated_dudx(k,k)*testp[l]*W*hang_weight; // voidage * du/dx term
         residuals[local_eqn] += interpolated_u[k] * dvoidagedx[k]*testp[l]*W*hang_weight;
            ///----------------------------------- Added void_ratio* ----------------------------------------
        }
             ...
             for(unsigned i2=0;i2<DIM;i2++)
              {
               ...
               if(local_unknown >= 0)
                {
         ///----------------------------------- Added by Mitchel -----------------------------------------
                 jacobian(local_eqn,local_unknown) += void_ratio*dpsifdx(l2,i2)*testp[l]*W*hang_weight*hang_weight2;
                  /// Also add the contribution of u*dvoidagedx here?
         ///----------------------------------- Added void_ratio* ----------------------------------------
                }
              }
            }
...
//======================================================================
template <unsigned DIM>
void RefineableAndersonJacksonEquations<DIM>::get_dresidual_dnodal_coordinates(
...
///----------------------------------- Added by Mitchel -----------------------------------------
               ...
               for(unsigned k=0;k<DIM;k++)
                {
                 double tmp=void_ratio*scaled_re*interpolated_u[k];
                 if (!this->ALE_is_disabled)
                  {
                   tmp-=void_ratio*scaled_re_st*mesh_velocity[k];
                  }
                 ...
                 for(unsigned p=0;p<DIM;p++)
                  {
                   ...
                   if (i==p)
                    {
///----------------------------------- Added by Mitchel -----------------------------------------
                     sum-=void_ratio*scaled_re_st*val_time_weight*psif(q_local)*testf(l);
                     for (unsigned k=0;k<DIM;k++)
                      {
                       ...
                       sum-=void_ratio*tmp*dpsifdx(q_local,k)*testf(l);
                      }
                    }
///----------------------------------- Added void_ratio* ------------------------------------------
...
//======================================================================
template<>
void PRefineableAJQCrouzeixRaviartElement<2>::further_build()
{
 if (this->tree_pt()->father_pt()!=0)
  {
   //Call the generic further build (if there is a father)
   RefineableAndersonJacksonEquations<2>::further_build();
  }
 ...
 if (this->internal_data_pt(this->P_nst_internal_index)->nvalue()
    ...
    if(this->tree_pt()->father_pt()!=0)
  {
   ...
   PRefineableAJQCrouzeixRaviartElement<2>* father_element_pt=
    dynamic_cast<PRefineableAJQCrouzeixRaviartElement<2>*>
...
//======================================================================
template<>
void PRefineableAJQCrouzeixRaviartElement<3>::further_build()
{
 if (this->tree_pt()->father_pt()!=0)
  {
   //Call the generic further build (if there is a father)
   RefineableAndersonJacksonEquations<3>::further_build();
  }
 // Now do the PRefineableQElement further_build()
 ...
 if (this->internal_data_pt(this->P_nst_internal_index)->nvalue()
  ...
  {
   // Pointer to my father (in C-R element impersonation)
   PRefineableAJQCrouzeixRaviartElement<3>* father_element_pt=
    dynamic_cast<PRefineableAJQCrouzeixRaviartElement<3>*>
...
//====================================================================
template class RefineableAndersonJacksonEquations<2>;
template class RefineableAndersonJacksonEquations<3>;
template class RefineableAJQTaylorHoodElement<2>;
template class RefineableAJQTaylorHoodElement<3>;
template class RefineableAJQCrouzeixRaviartElement<2>;
template class RefineableAJQCrouzeixRaviartElement<3>;
template class PRefineableAJQCrouzeixRaviartElement<2>;
template class PRefineableAJQCrouzeixRaviartElement<3>;
}
