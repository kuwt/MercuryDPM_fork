...
#ifndef MERCURY_UNDERRESOLVEDCOUPLING_H
#define MERCURY_UNDERRESOLVEDCOUPLING_H
...
class UnderResolvedCoupling : public OomphDieFilling<RefineableAJQCrouzeixRaviartElement<3> >, public MercuryDieFilling
{
public:
            ...
            adaptEveryNFluidTimesteps_(5), updateCouplingEveryNParticleTimesteps_(1)
    {
        ...
        Problem::mesh_pt() = new RefineableSimpleCubicMesh<RefineableAJQCrouzeixRaviartElement<3> >(nx_,ny_,nz_,xMin_,xMax_,yMin_,yMax_,zMin_,zMax_,time_stepper_pt);
        // Set error estimator
        ...
        {
            auto currEl = dynamic_cast<RefineableAJQCrouzeixRaviartElement<3> *>(mesh_pt()->element_pt(e));
        ...
        std::cout <<"Number of equations: " << assign_eqn_numbers() << endl;
    ...
    RefineableSimpleCubicMesh<RefineableAJQCrouzeixRaviartElement<3>>* mesh_pt()
    {
        return dynamic_cast<RefineableSimpleCubicMesh<RefineableAJQCrouzeixRaviartElement<3>>*>(Problem::mesh_pt());
    }
    ...
    virtual void pinBC();
    virtual void setBC();
    void solveSystem(DocInfo &doc_info);
    ...
    void stickLeavesOfNeighbourRecursiveInDir(Tree* mainTree_, const int &iTree, std::vector<Tree*> &allNeighbouringLeaves, const int &dir_);
    void stickLeavesOfEqualOrSmallerSizedNeighbourRecursiveInDir(Tree* neighbourTree, std::vector<Tree*> &allNeighbouringLeaves, const int &dir_);
    void getInternalIndex(Tree* Tree_, int &internalIndex);

    void getNeighbourIndex(int indexTree_, int dir_, int &indexInternalNeighbour_);
    void getExternalNeighbour(Tree* Tree_, const int &iTree_, int dir_, int &differenceInLevel_, std::vector<int> &indexPath_, Tree* &neighbourPtr_);
    int convertIndexFromPath(std::vector<int> &indexPath_, const int &differenceInLevel_, const int &dir_);

    std::vector<Tree*> getNeighboursOfEl(const int &iEl_, const int &dir_) {return listOfRefinableNeighbours_[iEl_][dir_];}
    ...
    std::vector< std::vector< std::vector<Tree*> > > listOfRefinableNeighbours_;
    
    oomph::Vector<Tree*> prevTrees_;
    oomph::Vector<Tree*> prevLeaves_;
...
namespace getDataFromElement
{
    ...
    double getVoidageOfElement(const double& time, const oomph::Vector<double>& x)
    {
        ...
        for (unsigned iEl = 0; iEl < nEl; iEl++)
        {
            ...
            dynamic_cast<RefineableAJQCrouzeixRaviartElement<3>*>(ptrToCoupledClass->mesh_pt()->element_pt(
...
#include "UnderResolvedCoupling.cc"

#endif //UNDERRESOLVEDCOUPLING
